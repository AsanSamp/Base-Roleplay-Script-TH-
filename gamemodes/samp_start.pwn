...
#define SQL_PASSWORD "0622647041"
...
#define FACTION_TYPE_GOVERMENT (2)
...
{		
	...
	{	
		...
		SendClientMessageEx(playerid, COLOR_YELLOWEX, "UCP %s ไม่ได้ทำการสมัครในระบบอยู่แล้ว", ReturnName(playerid));
		SendClientMessage(playerid, -1, "กรุณาไปสมัคร UCP ในเว็บไซต์ก่อน");		
		KickEx(playerid);
		return 1;
	}
	...
	SendClientMessageEx(playerid, COLOR_YELLOW, "ยินดีตอนรับเข้าสู่ เซืฟเวอร์, %s {FFFFFF}["SCRIPT_REV"]", ReturnName(playerid));
	return ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "ยินดีตอนรับเข้าสู่เซืฟเวอร์", "SERVER: คุณฒีเวลา 60 วินาทีในการ เข้าสู่ระบบ\nทิป: หากผมเจอบัคให้ทำการิดต่อผู้ดูแลระบบ\n           ใส่รหัสผ่าน:", "ตกลง", "ยกเลิก");
}
...
function:OnSecretWordInput(playerid)
{
	if(!cache_num_rows())
	{
		SendClientMessage(playerid, COLOR_RED, "ACCESS DENIED:{FFFFFF} คุณใส่รหัสผ่านความปลอดภัย ไม่รวบรัดอาจะทำให้ไม่มีความปลดภัยได้");
		KickEx(playerid);
		return 1;
	}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch (dialogid)
	{
		...
		case DIALOG_LOGIN:
		{
			if (!response)
			{
				SendClientMessage(playerid, COLOR_REDEX, "คุณถูกแตะ ออกจากเซืฟเวอร์เนื่องจากคุณ ยกเลิกในการ เข้าสู่ระบบ"); 
				return KickEx(playerid);
			}
		...
		case DIALOG_CHARS:
		{
			if (!response)
			{
				SendClientMessage(playerid, COLOR_REDEX, "คุณถูกเตะออกจากเซืฟเวอร์เนื่องจาก ไม่ได้เลือกตัวละคร");
				return KickEx(playerid); 
			}
			
			if(strfind(characterLister[playerid][listitem], "[", false) != -1)
			{
				SendClientMessage(playerid, COLOR_REDEX, "กรุณาไปสมัครตัวละครในเว็บไซต์");
				return KickEx(playerid);
			}
			else
			{
				...
				format(string, sizeof(string), "คุณเข้าสู่ระบบด้วยชื่อ {145F0B}%s{FFFFFF}.", characterLister[playerid][listitem]);
		...
		case DIALOG_FACTION_CONFIG:
		{
			if(response)
			{
				switch(listitem)
				{
					case 0: return ShowPlayerDialog(playerid, DIALOG_FACTION_NAME, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่ชื่อเฟคชั่นของคุณ:", "Select", "<<"); 
					case 1: return ShowPlayerDialog(playerid, DIALOG_FACTION_ABBREV, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่ชื่อย่อเฟคชั่นของคุณ:", "Select", "<<"); 
					case 2: 
					{
						if(PlayerInfo[playerid][pFactionRank] != 1)
						{
							SendClientMessage(playerid, COLOR_RED, "ACCESS DENIED:{FFFFFF} การจะแก้ไขจำเป็นต้องใช้ ยศ/ต่ำแหน่ง ที่ 1 เท่านั้น");
							return ShowFactionConfig(playerid);
						}
						
						return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_R, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่เลขยศ/ต่ำแหน่งที่จะให้สามารถแก้ไขได้:", "ตกลง", "<<"); 
					}
					case 3: return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_J, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่เลขยศ/ต่ำแหน่งที่จะให้เป็นยนเริ่มต้น:", "ตกลง", "<<");
					case 4: return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_C, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่เลขยศ/ต่ำแหน่งที่จให้พิมพ์แชทกลุ่มได้", "ตกลง", "<<"); 
					case 5: return ShowPlayerDialog(playerid, DIALOG_FACTION_CHATCOLOR, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่โค้ดสีของแชท: (Example: 0x8D8DFFFF)", "ตกลง", "<<");
					case 6: return ShowPlayerDialog(playerid, DIALOG_FACTION_RANKS, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่เลขยศ/ต่ำแหน่ง ที่คุณต้องการแก้ไข(1-20)", "ตกลง", "<<");
					case 7:
					{
						if(PlayerInfo[playerid][pFactionRank] != 1)
							return SendErrorMessage(playerid, "ACCESS DENIED:{FFFFFF} การจะแก้ไขจำเป็นต้องใช้ ยศ/ต่ำแหน่ง ที่ 1 เท่านั้น");
						...
						if(GetPlayerInterior(playerid) != 0)
							FactionInfo[factionid][eFactionSpawnWorld] = GetPlayerVirtualWorld(playerid); 
					...
					case 8: return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_T, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่เลขยศ/ต่ำแหน่ง ที่คุณต้องการให้รียานพาหนะได้:", "ตกลง", "<<"); 
				}
			}
		}
		...
		case DIALOG_FACTION_ABBREV:
		{
			...
			if(strlen(inputtext) > 30 || strlen(inputtext) < 1)
				return ShowPlayerDialog(playerid, DIALOG_FACTION_NAME, DIALOG_STYLE_INPUT, "Faction Configuration", "ชื่อย่อเฟคชั่นของคุณควรไม่เกิน 30 ตัวอักษร\n\nใส่ชื่อย่อเฟคชั่นของคุณ:", "ตกลง", "<<"); 
		...
		case DIALOG_FACTION_ALTER_R:
		{
			...
			if(rankid > 20 || rankid < 1)
				return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_R, DIALOG_STYLE_INPUT, "Faction Configuration", "เลขยศ/ตำแหน่ง ต้องไม่เกิน 1-20\n\nใส่เลขยศ/ต่ำแหน่งที่จะให้สามารถแก้ไขได้", "ตกลง", "<<"); 
		...
		case DIALOG_FACTION_ALTER_J:
		{
			...
			if(strlen(inputtext) < 1 || strlen(inputtext) > 2)
				return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_J, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่เลขยศ/ต่ำแหน่งที่จะให้เป็นยนเริ่มต้น:", "Select", "<<");

			new rankid = strval(inputtext); 
			
			if(rankid > 20 || rankid < 1)
				return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_J, DIALOG_STYLE_INPUT, "Faction Configuration", "เลขยศ/ต่ำแหน่งต้องไม่เกิน 1-20.\n\nใส่เลขยศ/ต่ำแหน่งที่จะให้เป็นยนเริ่มต้น:", "ตกลง", "<<"); 
		...
		case DIALOG_FACTION_ALTER_C:
		{
			...
			if(strlen(inputtext) < 1 || strlen(inputtext) > 2)
				return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_C, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่เลขยศ/ต่ำแหน่งที่จให้พิมพ์แชทกลุ่มได้:", "Select", "<<"); 

			new rankid = strval(inputtext); 
			
			if(rankid > 20 || rankid < 1)
				return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_C, DIALOG_STYLE_INPUT, "Faction Configuration", "เลขยศ/ต่ำแหน่งต้องไม่เกิน 1-20.\n\nใส่เลขยศ/ต่ำแหน่งที่จให้พิมพ์แชทกลุ่มได้:", "ตกลง", "<<"); 
		...
		case DIALOG_FACTION_ALTER_T:
		{
			...
			if(strlen(inputtext) < 1 || strlen(inputtext) > 2)
				return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_T, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่เลขยศ/ต่ำแหน่งที่จะสามารถ รียานพาหนะได้:", "ตกลง", "<<"); 
				
			new rankid = strval(inputtext); 
			
			if(rankid > 20 || rankid < 1)
				return ShowPlayerDialog(playerid, DIALOG_FACTION_ALTER_T, DIALOG_STYLE_INPUT, "Faction Configuration", "เลขยศ/ต่ำแหน่งต้องไม่เกิน 1-20.\n\nใส่เลขยศ/ต่ำแหน่งที่จะสามารถ รียานพาหนะได้:", "ตกลง", "<<");
		...
		case DIALOG_FACTION_CHATCOLOR:
		{
			...
			SendServerMessage(playerid, "คุณได้เปลี่ยนสีแชทเฟคชั่นของคุณ");
		...
		case DIALOG_FACTION_RANKS:
		{
			...
			if(rankid > 20 || rankid < 1)
				return ShowPlayerDialog(playerid, DIALOG_FACTION_RANKS, DIALOG_STYLE_INPUT, "Faction Configuration", "ใส่เลขยศ/ต่ำแหน่ง ที่จะแก้ไขชื่อ ยศ/ต่ำแหน่ง", "ตกลง", "<<");
				
			playerEditingRank[playerid] = rankid;
			
			format(str, sizeof(str), "คุณกำลังแก้ไขยศ/ต่ำแหน่ง %i ('%s').\n\n{F81414}หากจะลบชื่อยศใหตั้งเป็น \"NotSet\". ", rankid, FactionRanks[PlayerInfo[playerid][pFaction]][rankid]);
			return ShowPlayerDialog(playerid, DIALOG_FACTION_RANKEDIT, DIALOG_STYLE_INPUT, "Faction Configuration", str, "ตกลง", "<<"); 
		}
		case DIALOG_FACTION_RANKEDIT:
		{
			...
			SendServerMessage(playerid, "คุณได้แก้ไขชือยศ %i (%s) เป็น: \"%s\". ", playerEditingRank[playerid], FactionRanks[PlayerInfo[playerid][pFaction]][playerEditingRank[playerid]], inputtext);
		...
		case DIALOG_HOUSE_WEAPONS:
		{
			if(response)
			{
				...
				format(str, sizeof(str), "* %s หยิบ %s ออกมาจากตู้เซฟ", ReturnName(playerid, 0), ReturnWeaponName(PropertyInfo[id][ePropertyWeapons][listitem+1])); 
				SetPlayerChatBubble(playerid, str, COLOR_EMOTE, 20.0, 4500); 
				SendClientMessage(playerid, COLOR_EMOTE, str); 
		...
		case DIALOG_XMR_CATEGORIES:
		{
			if(response)
			{
				...
				strcat(liststr, "{FFE104}OFF - คลิกเพื่อปิด\n"); 
		...
		case DIALOG_XMR_STATIONS:
		{
			if(response)
			{
				...
				if(!strcmp(inputtext, "OFF - คลิกเพื่อปิด."))
				{
			...
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						...
						{
							SendClientMessage(playerid, COLOR_ACTION, "คุณมีวิทยุอยู่แล้ว");
						...
						{
							SendClientMessage(playerid, COLOR_ACTION, "คุณมีเงินไม่เพียงพอ");
							ShowBuylist(playerid);
							return 1; 
						}
					
						SendClientMessage(playerid, COLOR_ACTION, "คุณได้ซื้อวิทยุในราคา $1,000.");
					...
					case 1:
					{
						...
						{
							SendClientMessage(playerid, COLOR_ACTION, "คุณมีเงินไม่พอที่จะซื้อน้ำมันสำรอง");
							ShowBuylist(playerid);
							return 1; 
						}
						...
						{
							SendClientMessage(playerid, COLOR_ACTION, "คุณมีนำมันสำรองเต็มตัวแล้ว");
							ShowBuylist(playerid);
							return 1;
						}
						
						SendClientMessage(playerid, COLOR_ACTION, "คุณได้ซื้อน้ำมันสำรองในราคา $500.");
						GiveMoney(playerid, -500); 
					...
					case 2:
					{
						if(5000 > PlayerInfo[playerid][pMoney])
						{
							SendClientMessage(playerid, COLOR_ACTION, "คุณมีเงินไม่พอที่จะซื้อหน้ากาก");
							ShowBuylist(playerid);
							return 1; 
						}
						...
						{
							SendClientMessage(playerid, COLOR_ACTION, "คุณมีหน้ากากอยู่แล้ว");
							ShowBuylist(playerid);
							return 1; 
						}
						
						SendClientMessage(playerid, COLOR_ACTION, "คุณได้ซื้อหน้ากากในราคา $5,000.");
		...
		case DIALOG_DEALERSHIP_SELECT:
		{
			...
			if(g_aDealershipData[i][eDealershipPrice] > PlayerInfo[playerid][pMoney])
				return SendServerMessage(playerid, "ต้องการเงิน $%s ในการซื้อรถ. (รวม: $%s)", MoneyFormat(g_aDealershipData[i][eDealershipPrice]), MoneyFormat(PlayerInfo[playerid][pMoney])); 
			...
			if(response)
			{
				switch(listitem)
				{
					...
					case 5: //Purchase
					{
						...
						ShowPlayerDialog(playerid, DIALOG_DEALERSHIP_PURCHASE, DIALOG_STYLE_MSGBOX, caption, str, "แก้ไข", "จ่ายเงิน"); 
						return 1;
					}
				}
			}
			else ConfirmDialog(playerid, "Confirmation", "คุณแน่ใจนะว่าจะยกเลิกการซื้อรถ?", "OnPlayerExitDealership"); 
			return 1;
		}
		...
		{		
			...
			else
			{
				...
				{
					SendServerMessage(playerid, "คุณไม่สามารถซื้อได้เนื่องจากเงินคุณยังมีไม่เพียงพอ (ราคา: $%s, รวม:$%s)", MoneyFormat(DealershipTotalCost[playerid]), MoneyFormat(PlayerInfo[playerid][pMoney]));
		...
		case DIALOG_DEALERSHIP_APPEND_COLORMENU:
		{
			...
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						format(str, sizeof(str), "{FFFFFF}คุณกำลังเลือกสีหลักของยานพาหนะคุณ. ({%06x}%d{FFFFFF})\n โปรดใส่ไอดีสีที่คุณต้องการ. (0-255)", VehicleColoursTableRGBA[DealershipCarColors[playerid][0]] >>> 8, DealershipCarColors[playerid][0]); 
						ShowPlayerDialog(playerid, DIALOG_DEALERSHIP_APPEND_COLOR1, DIALOG_STYLE_INPUT, caption, str, "ตกลง", "<<");  
					}
					case 1:
					{
						format(str, sizeof(str), "{FFFFFF}คุณกำลังเลือกสีรองของยานพาหนะคุณ. ({%06x}%d{FFFFFF})\n โปรดใส่ไอดีสีที่คุณต้องการ. (0-255)", VehicleColoursTableRGBA[DealershipCarColors[playerid][1]] >>> 8, DealershipCarColors[playerid][1]); 
						ShowPlayerDialog(playerid, DIALOG_DEALERSHIP_APPEND_COLOR2, DIALOG_STYLE_INPUT, caption, str, "ตกลง", "<<"); 
					}
				}
			}
			else return ShowDealerAppend(playerid);
		...
		case DIALOG_DEALERSHIP_APPEND_COLOR1:
		{
			...
			if(response)
			{
				...
				{
					format(str, sizeof(str), "{FFFFFF}คุณกำลังเลือกสีรองของยานพาหนะคุณ ({%06x}%d{FFFFFF})\n โปรดใส่ไอดีสีที่คุณต้องการ. (0-255)", VehicleColoursTableRGBA[DealershipCarColors[playerid][0]] >>> 8, DealershipCarColors[playerid][0]); 
					return ShowPlayerDialog(playerid, DIALOG_DEALERSHIP_APPEND_COLOR1, DIALOG_STYLE_INPUT, caption, str, "ตกลง", "<<");  
				}
			...
			else
			{
				...
				ShowPlayerDialog(playerid, DIALOG_DEALERSHIP_APPEND_COLORMENU, DIALOG_STYLE_LIST, caption, str, "ตกลง", "<<"); 
			}
			return 1;
		}
		...
		case DIALOG_SELECT_HOUSE:
		{
			if(response)
			{
				...
				SendServerMessage(playerid, "คุณได้เลือกว่าจะเกิดที่บ้าน %i และต่อไปคุณจะเกิดที่นี่", listitem); 
			}
			return 1;
		}
		case DIALOG_MDC:
		{
			if(response)
			{
				switch(listitem)
				{
					case 0: return ShowPlayerDialog(playerid, DIALOG_MDC_NAME, DIALOG_STYLE_INPUT, "Name Search - MDC", "พิมพ์ชื่อจริงลงไปเพื่อทำการค้นหา (ตัวอย่าง:Firstname_Lastname):", "ค้นหา", "<<"); 
					case 1: return ShowPlayerDialog(playerid, DIALOG_MDC_PLATE, DIALOG_STYLE_INPUT, "Plate Search - MDC", "พิมพ์ป้ายทะเบียนลงไป:", "ค้นหา", "<<"); 
				}
				return 1;
			}
		}
		case DIALOG_MDC_NAME:
		{
			if(response)
			{
				if(strlen(inputtext) < 3 || strlen(inputtext) > 32)
					return ShowPlayerDialog(playerid, DIALOG_MDC_NAME, DIALOG_STYLE_INPUT, "Name Search - MDC", "พิมพ์ชื่อจริงลงไปเพื่อทำการค้นหา (ตัวอย่าง:Firstname_Lastname):", "ค้นหา", "<<"); 
					
				for(new i = 0; i < strlen(inputtext); i++)
				{
					if(inputtext[i] == '_')
					{
						return ShowPlayerDialog(playerid, DIALOG_MDC_NAME, DIALOG_STYLE_INPUT, "Name Search - MDC", "พิมพ์ป้ายทะเบียนลงไป:", "ค้นหา", "<<"); 
					}
					else
					{
						if(inputtext[i] == ' ')
				...
				ShowPlayerDialog(playerid, DIALOG_MDC_NAME_QUEUE, DIALOG_STYLE_MSGBOX, "Name Search - MDC", "ค้นหา.", "ยกเลิก", ""); 
				return 1;
			}
		...
		case DIALOG_MDC_PLATE:
		{
			if(response)
			{
				if(strlen(inputtext) > 6 || strlen(inputtext) < 3)
					return ShowPlayerDialog(playerid, DIALOG_MDC_PLATE, DIALOG_STYLE_INPUT, "Plate Search - MDC", "พิมพ์ป้ายทะเบียนลงไป:\n\nต้องพิมพ์ป้ายทะเบียนให้ถูก(มากกว่า 3 ตัว).", "ค้นหา", "<<"); 
					
				PlayerMDCCount[playerid] = 0;
				PlayerMDCTimer[playerid] = SetTimerEx("OnMDCSearch", 1000, true, "ii", playerid, 2);
				
				format(PlayerMDCName[playerid], 32, "%s", inputtext); 
				ShowPlayerDialog(playerid, DIALOG_MDC_PLATE_QUEUE, DIALOG_STYLE_MSGBOX, "Plate Search - MDC", "ค้นหา.", "ยกเลิก", ""); 
			}
			else return ShowPlayerMDC(playerid);
		}
		case DIALOG_MDC_NAME_QUEUE:
		{
			if(response)
			{
				...
				format(str, sizeof(str), "{FFFFFF}ยกเลิการค้นหา: \"%s\"", PlayerMDCName[playerid]);
		...
		case DIALOG_MDC_PLATE_QUEUE:
		{
			if(response)
			{
				...
				format(str, sizeof(str), "{FFFFFF}ยกเลิกการค้นหา: \"%s\"", PlayerMDCName[playerid]);
...
function:OnMDCSearch(playerid, type)
{
	...
	switch(PlayerMDCCount[playerid])
	{
		...
		case 2: str = "ค้นหา...";
		case 3: str = "ค้นหา...."; 
		case 4: str = "ค้นหาสำเร็จ กำลังแสดงข้อมูล."; 
		case 5:
		{
			KillTimer(PlayerMDCTimer[playerid]);
			PlayerMDCCount[playerid] = 0;
	...
	ShowPlayerDialog(playerid, dialog_id, DIALOG_STYLE_MSGBOX, caption, str, "ยกเลิก", ""); 
	return 1;
}
...
function:OnMDCRecordSearch(playerid, type)
{
	...
	switch(type)
	{
		case 1: //Name search;
		{
			...
			{
				return ShowPlayerDialog(playerid, DIALOG_DEFAULT, DIALOG_STYLE_MSGBOX, "Name Search - MDC", "{FFFFFF}ไม่มีชื่อที่คุณค้นหาอยู่ในฐานระบบ.", "<<", ""); 
			}
...
{	
	if(!cache_num_rows())
	{
		...
		return ShowPlayerDialog(playerid, DIALOG_DEFAULT, DIALOG_STYLE_MSGBOX, "Plate Search - MDC", "{FFFFFF}ไม่มีป้ายทะเบียนที่คุณค้นหา.", "<<", ""); 
	}
	...
	ShowPlayerDialog(playerid, DIALOG_MDC_PLATE_LIST, DIALOG_STYLE_LIST, "Plate Search - MDC", str, "ตกลง", "<<"); 
	return 1;
}
...
function:OnMDCNameFound(playerid)
{
	...
	format(sub_str, sizeof(sub_str), "ชื่อจริง-นามสกุล: %s\n", PlayerMDCName[playerid]);
	strcat(primary_str, sub_str);
	
	format(sub_str, sizeof(sub_str), "เบอร์ดทรสัพท์: %i\n\n", phone_number);
	strcat(primary_str, sub_str);
	
	format(sub_str, sizeof(sub_str), "วันเกิด: %s\n", age_str);
	strcat(primary_str, sub_str);
	
	format(sub_str, sizeof(sub_str), "สถาณที่เกิด: %s\n\n", origin_str);
	strcat(primary_str, sub_str);
	
	strcat(primary_str, "CRIMINAL RECORD\n");
	
	format(sub_str, sizeof(sub_str), "ถูกขังคุก: %i\n", jail_times);
	strcat(primary_str, sub_str);
	
	format(sub_str, sizeof(sub_str), "ถูกขังเรือนจำ: %i\n\n", prison_times);
	strcat(primary_str, sub_str);
	
	if(!active_listing)
		strcat(primary_str, "คดีติดตดัว(s): (NOT WANTED)\n\n");
...
function:LoggingIn(playerid)
{
	if(!cache_num_rows())
	{
		...
		{
			SendClientMessage(playerid, COLOR_REDEX, "[SERVER]: คุณถูกเตะออกเนื่องจากคุณใส่รหัสผ่านผิดครบ 3 ครั้งแล้ว");
			return KickEx(playerid);
		}
		
		return ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "ยินดีตอนรับเข้าสู่เซืฟเวอร์", "คุณใส่รหัสผ่านผิด\n\nSERVER: คุณมีเวลา 60 วินาทีในการเข้าสู่ระบบ\nทิป: หากพบเจอบัคให้แจ้งผู้ดูแลระบบ\nผ่านทางฟอรั่ม\n\n           ใส่รหัสผ่าน:", "ตกลง", "ยกเลิก");
	}
	...
	{
		ShowPlayerDialog(playerid, DIALOG_SECRETWORD_CREATE, DIALOG_STYLE_PASSWORD, "ยินดีตอนรับเข้าสู่เซฟเวอร์",
		"รหัสรักษาความปลอดภัย:\n\nเราจำเป็นต้องสร้างรหัสที่เป็นความรักษาความปลอดภัยของ ไอดี ซื้อมีความจำเป็นในการที่จะต้องใส่รหัสผ่านนี้เข้าไป\n\nและคุณจำเป็นต้องจำรหัสผ่านนี้ไว้\n\n{F81414}ขอให้แน่ใจว่ารหัสผ่านจะไม่ตรงกับรหัสผ่านแรกของคุณ", "เข้าใจ", "ยกเลิก");
		return 1; 
	}
...
function:Query_CheckBannedAccount(playerid)
{
	...
	else
	{
		...
		SendServerMessage(playerid, "บัญชี UCP %s ของคุณ", ReturnName(playerid));
		SendServerMessage(playerid, "ถูกแบบออกจากเซืฟเวอร์ ในวันที่:%s โดย %s.", banDate, banner); 
		return KickEx(playerid);
	}
	return 1;
}
...
function:DB_ListCharacters(playerid)
{
	if(!cache_num_rows())
	{
		SendClientMessage(playerid, COLOR_YELLOW, "ตัวละครของคุณว่างกล่าว");
		
		new listStrings[128];
		
		for (new i = 0; i < 1; i++)
		{
			characterLister[playerid][i] = "[ตัวละคร ว่างปล่าว]"; 
			format(listStrings, sizeof(listStrings), "%s%s\n", listStrings, characterLister[playerid][i]);
		}
		
		return ShowPlayerDialog(playerid, DIALOG_CHARS, DIALOG_STYLE_LIST, "ตัวละคร:", listStrings, "ตกลง", "<<"); 
	}
	...
	if (e_pAccountData[playerid][mLoggedin] == false)
	{
		...
		format(string, sizeof(string), "เข้าสู่ระบบด้วยชื่อ {145F0B}%s{FFFFFF}.", ReturnName(playerid));
		SendClientMessage(playerid, -1, string);
	...
	if(rows)
	{
		...
		for (new i = 0; i < 1; i++)
		{
			...
			{
				characterLister[playerid][i] = "[ตำละคนว่างปล่าว]"; 
			}
			
			format(listString, sizeof(listString), "%s%s\n", listString, characterLister[playerid][i]);
		}
		
		ShowPlayerDialog(playerid, DIALOG_CHARS, DIALOG_STYLE_LIST, "ตัวละคร:", listString, "ตกลง", "<<"); 
	}
	return 1;
}
...
function:Query_SelectCharacter(playerid)
{
	if (!cache_num_rows())
	{
		SendClientMessage(playerid, -1, "[ERROR]: เกิดข้อผิดผลาดกับตัวละครของคุณ โปรดติดต่อผู้ดูแลระบบ");
		...
		return ShowPlayerDialog(playerid, DIALOG_CHARS, DIALOG_STYLE_LIST, "ตัวละคร:", listString, "ตกลง", "<<"); 
	}
...
function:LoadCharacter(playerid)
{
	...
	{
		SendClientMessageEx(playerid, COLOR_WHITE, "SERVER: คุณได้เข้าสู่ระบบ ตัวละครที่เป็น ผู้ดูแลระบบ ระดับ %i", PlayerInfo[playerid][pAdmin]);
		
		if(!strcmp(e_pAccountData[playerid][mForumName], "Null"))
		{
			ShowPlayerDialog(playerid, 99, DIALOG_STYLE_MSGBOX, "การแจ้งเตือน", "ข้อความนี้ส่งให้ ผู้ดูแลระบบ ทุกคนที่ยังไม่ได้ทำการตั้งชื่อ 'Forum' ภายในเซืฟเวอร์\nกรุณาเปลี่ยนชื่อ ฟอรั่มให้เป็นชื่อที่ถูกต้องตามกฏด้วย\n\n{F81414}ทำการพิมพ์ /forumname เพื่อทำการเปลี่ยนชื่อ 'forum' ของคุณ", "เข้าใจแล้ว", ""); 
		}
	}
	...
	if(PlayerInfo[playerid][pVehicleSpawned] == true)
	{
		if(!IsValidVehicle(PlayerInfo[playerid][pVehicleSpawnedID]))
			...
			SendServerMessage(playerid, "รถของคุณ ยังไม่ได้อยู่ในที่จัดเก็บ");
	}
	...
	if(PlayerInfo[playerid][pOfflinejailed])
	{
		...
		{
			SendClientMessageToAllEx(COLOR_RED, "AdmCmd: %s ได้ถูกส่งคุกแอดมินเป็นเวล %d โดย ระบบ สาเหตุ: %.56s", ReturnName(playerid), PlayerInfo[playerid][pAdminjailTime] / 60, PlayerInfo[playerid][pOfflinejailedReason]);
			SendClientMessageToAllEx(COLOR_RED, "AdmCmd: ...%s", PlayerInfo[playerid][pOfflinejailedReason][56]); 
		}
		else SendClientMessageToAllEx(COLOR_RED, "AdmCmd: %s ได้ถูกส่งคุกแอดมินเป็นเวล %d โดย ระบบ สาเหตุ: %s", ReturnName(playerid), PlayerInfo[playerid][pAdminjailTime] / 60, PlayerInfo[playerid][pOfflinejailedReason]);
	...
	foreach (new i : Player)
	{
		...
		if(e_pAccountData[i][mLoggedin] == false)
		{
			if(loginTime[i] > 0)
			{
				...
				if(loginTime[i] >= 60)
				{
					SendServerMessage(i, "คุณถูกตะออกจาเซืฟเวอร์เนื่องจากไม่ทำการ เข้าสู่ระบบในเวลาที่กำหนด"); 
					KickEx(i); 
				}
			}
		}
		...
		if(characterPickTime[i] > 0)
		{
			...
			{
				SendServerMessage(i, "คุณถูกเตะออกจากเซืฟเวอร์เนื่องจากไม่ได้เลือกตัวละคร");
				KickEx(i);
			}
		}
		...
		if(PlayerTakingLicense[i] && PlayerLicenseTime[i] <= 60)
		{
			...
			{
				StopDriverstest(i);
				SendClientMessage(i, COLOR_DARKGREEN, "คุณขับรถนานเกินไปกว่าเวลาที่กำหนด"); 
			}
		}
	}
	return 1;
...
public OnVehicleDeath(vehicleid, killerid)
{
	...
	else
	{
		...
		{
			SendClientMessageEx(i, COLOR_RED, "ยานพาหนะ %s ถูกทำลาย", ReturnVehicleName(vehicleid)); 
	...
	for (new i = 1, j = GetVehiclePoolSize(); i <= j; i++)
	{
		...
		if(vehicle_health <= 248) //this is the point where the vehicle starts to catch fire;
		{
			...
			{			
				...
				SendClientMessageEx(owner_id, COLOR_RED, "ยานพาหนะของคุณ %s ถูกทำลาย", ReturnVehicleName(i)); 
			...
			if(car_driver == INVALID_PLAYER_ID)
				format(str, sizeof(str), "ยานพาหนะ (%s) ถูกทำลาย", ReturnVehicleName(i), i); 
			
			else format(str, sizeof(str), "ยานพาหนะ %s (ไอดี: %i) ถูกทำลาย: %s", ReturnVehicleName(i), i, ReturnName(car_driver));
...
function:CallPaycheck()
{
	foreach(new i : Player)
	{
		...
		total_tax = PlayerInfo[i][pBank] / 500; 
	...
	foreach(new i : Player)
	{
		...
		for(new p = 1; p < MAX_PROPERTY; p++)
		{
			...
			if(IsPlayerInRangeOfPoint(i, 3.0, PropertyInfo[p][ePropertyEntrance][0], PropertyInfo[p][ePropertyEntrance][1], PropertyInfo[p][ePropertyEntrance][2]))
			{
				...
				{
					SendClientMessage(i, COLOR_DARKGREEN, "คุณอยู่หน้าบ้านสามารถพิมพ์ /buyproperty."); 
					SendClientMessageEx(i, COLOR_DARKGREEN, "ในราคา: $%i", PropertyInfo[p][ePropertyMarketPrice]);
				}
				else
				{
					SendClientMessageEx(i, COLOR_DARKGREEN, "เจ้าของบ้านหลังนี้คือ %s สามารถ /enter เพื่อเข้าไปข้างใน",  ReturnDBIDName(PropertyInfo[p][ePropertyOwnerDBID]));
				}	
			}
		}
	}
...
function:FunctionCheats(playerid)
{
	...
	{
		format(str, sizeof(str), "ผู้เล่น %s (%i) มีการใช้โปรแกรมช่วยเล่น (Flying)", ReturnName(playerid), playerid);
		SendAdminMessage(1, str); 
	}
	return 1;
}
...
public OnPlayerDeath(playerid, killerid, reason)
{
	if(killerid != INVALID_PLAYER_ID)
		SendAdminMessageEx(COLOR_RED, 4, "[DEBUG DEBUG] %s ถูกสังหารโดย %s (%s)", ReturnName(playerid), ReturnName(killerid), ReturnWeaponName(reason));
...
public OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float:fX, Float:fY, Float:fZ)
{
	...
	if(hittype == BULLET_HIT_TYPE_PLAYER) //Taser system; 
	{
		if(PlayerInfo[playerid][pTaser] && weaponid == 23)
		{
			...
			SendNearbyMessage(hitid, 20.0, COLOR_EMOTE, "* %s ถูกยิงด้วยปืนช็อตไฟฟ้าและล้มลงไปกับพื้น", ReturnName(hitid, 0)); 
			GameTextForPlayer(hitid, "~b~You Are Tasered", 2500, 3);
			
			SendClientMessage(hitid, COLOR_YELLOWEX, "-> ในตอนนี้คุณถูกยิงด้วยปืนช็อไฟฟ้าคุณจะไม่สามารถขยับตัวได้ชั่วขณะ");
...
{	
	...
	if(PlayerInfo[playerid][pAdminjailed] == true)
	{
		...
		SendServerMessage(playerid, "คุณยังถูกส่งมาที่คุกแอดมินเพื่อมาอยู่ในนี้ให้ครบตามเวลาที่เหลือ %i นาที", PlayerInfo[playerid][pAdminjailTime] / 60);
	}
	else
	{
...
public OnPlayerText(playerid, text[])
{
	if(PlayerInfo[playerid][pSpectating] != INVALID_PLAYER_ID)
	{
		SendServerMessage(playerid, "คุณไม่สามารถพูดได้ในขณะที่กำลัง ส่อง");
		return 0;
	}
	
	if(PlayerInfo[playerid][pLoggedin] && GetPlayerTeam(playerid) != PLAYER_STATE_ALIVE)
	{
		SendServerMessage(playerid, "คุณไม่สามารถพูดได้ในขณะที่คุณได้รับบาดเจ็บ");
		return 0;
	}
	
	if(PlayerInfo[playerid][pRelogging])
	{
		SendServerMessage(playerid, "คุณไม่สามารถพูดได้ในขณะที่กำลังจะกลับไปหน้าเลือกตัวละคร");
		return 0;
	}
	
	if(!playerMakingCharacter[playerid] && !e_pAccountData[playerid][mLoggedin] && !PlayerInfo[playerid][pLoggedin])
	{
		SendServerMessage(playerid, "คุณไม่สามารถพูดได้ในขณะที่กำลังเข้าสู่ระบบ");
		return 0;
	}
...
public OnPlayerCommandReceived(playerid, cmdtext[])
{
	...
	{
		SendClientMessage(playerid, COLOR_RED, "ACCESS DENIED:{FFFFFF} คุณจะต้องเข้าสู่ระบบเพื่อใช้คำสั่ง");
...
public OnPlayerCommandPerformed(playerid, cmdtext[], success)
{
	if(!success)
	{
		if(strlen(cmdtext) > 28) // Preventing long bad commands from returning default message;
			SendServerMessage(playerid, "ขออภัยคำสั่งของคุณไม่มีอยู่ภายในเซืฟเวอร์ พิมพ์ /help เพื่อดูคำสั่ง"); 
			
		else
			SendServerMessage(playerid, "ขออภัยคำสั่ง %s ไม่มีอยู่ภายในเซืฟเวอร์ พิมพ์ /help เพื่อดูคำสั่ง", cmdtext);
	}
	else return 1;
	return 1;
}
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	if(newstate == PLAYER_STATE_DRIVER)
	{
		...
		if(VehicleInfo[GetPlayerVehicleID(playerid)][eVehicleOwnerDBID] == PlayerInfo[playerid][pDBID])
			SendClientMessageEx(playerid, COLOR_WHITE, "ยินดีตอนรับเข้าสู่รถ %s.", ReturnVehicleName(GetPlayerVehicleID(playerid)));
			
		for(new i = 0; i < sizeof dmv_vehicles; i++) if(GetPlayerVehicleID(playerid) == dmv_vehicles[i])
			SendServerMessage(playerid, "ตอนนี้คุณอยู่บนรถทดสอบใบขับขี่ พิมพ์ /licenseexam เพื่อเริ่มทำการทดสอบ"); 
	}
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	if(!ispassenger && VehicleInfo[vehicleid][eVehicleFaction] && PlayerInfo[playerid][pFaction] != VehicleInfo[vehicleid][eVehicleFaction])
	{
		...
		{
			playerVehicleTextdraw[playerid] = CreateDynamic3DTextLabel("รถคันนี้เป็นรถเฟคชั่น (Faction-vehicle)", COLOR_WHITE, 0.0, 0.0, 0.0, 10.0,  vehicleid, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid), playerid); 
		...
		switch(PlayerInfo[playerid][pEditingObject])
		{
			case 1:
			{
				if(response == EDIT_RESPONSE_CANCEL)
				{
					...
					SendServerMessage(playerid, "คุณออกจากการซื้อ Boombox"); 
					return 1;
				}
					
				if(response == EDIT_RESPONSE_FINAL)
				{
					ConfirmDialog(playerid, "Confirmation", "คุณแน่ใจใช่ไหมที่จะซื้อ 'BoomBox' ในราคา\nในราคา: $1,000", "OnPropertyBoombox", IsPlayerInProperty(playerid), x, y, z, rx, ry, rz); 
					return 1;
				}
			}
			...
			case 2:
			{
				...
				if(response == EDIT_RESPONSE_CANCEL)
				{
					...
					SendServerMessage(playerid, "คุณยกเลิก"); 
					return 1;
				}
				...
				if(response == EDIT_RESPONSE_FINAL)
				{
					...
					SaveProperty(id);
					SendServerMessage(playerid, "คุณได้ปรับตำแหน่งของ 'BoomBox' เรียบร้อยแล้ว"); 
					return 1;
				}
			}
		}
...
{
	SendClientMessage(playerid, COLOR_YELLOW, "คุณรีเฟรชตัวละครของคุณ"); 
...
CMD:report(playerid, params[])
{
	...
	format(showString, sizeof(showString), "{FFFFFF}คุณมันใจใช่หรือไม่ที่จะส่งรายงาย\n\n{F81414}โปรดจำไว้ว่าการรายงานผู้เล่นนั้นตามปกติแล้วควรจะเห็นเหตุการณ์พร้อมกับถ่าย ภาพ หรือ วิดิโอ ไว้เพื่อเป็นหลักฐานในการชี้ตัวว่าผู้เล่นดังกล่าวผิดจริง\nแต่ไม่ว่าอย่างไรก็ตามสิ่งที่ควรกระทำมากที่สุดคือการ ถ่าย หรือ อัดวิดิโอ ไว้เป็นหลักฐานและนำไปรายงานในฟอรั่มจะดีกว่าการรายงานผ่านทาง /report\n\n{FFFFFF}รายงานเรื่อง: %s", params);
	ShowPlayerDialog(playerid, DIALOG_REPORT, DIALOG_STYLE_MSGBOX, "ยืนยันการทำรายการ", showString, "ตกลง", "ยกเลิก"); 
...
CMD:relog(playerid, params[])
{
	if(gettime() - PlayerInfo[playerid][pLastDamagetime] < 120)
		return SendServerMessage(playerid, "คุณได้รับ ความเสียหายกับตัวละครของคุณ ไม่สามารถทำการ Relog ได้"); 
		
	if(PlayerInfo[playerid][pRelogging])
		return SendErrorMessage(playerid, "ตอนนี้คุณไม่สามารถใช้คำสั่งนี้ได้"); 
	...
	format(str, sizeof(str), "%s กำลังทำการ Relog เพื่อกลับไปหน้าเลือกตัวละคร", ReturnName(playerid));
	SendAdminMessage(1, str);
	...
	TogglePlayerControllable(playerid, 0); 
	SendServerMessage(playerid, "โปรดหลีกเลี่ยงการขัยบตัว หรือพูดคุยบทบาทต่างๆในเวลานี้"); 
	return 1;
}
...
CMD:pay(playerid, params[])
{
	...
	if(gettime() - playerLastpay[playerid] < 3)
		return SendServerMessage(playerid, "ตอนนี้คุณจ่ายเงินเยอะเกินไปโปรดรอก่อนที่จะทำการจ่ายใหม่อีกครั้ง");
		
	PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0); PlayerPlaySound(playerb, 1052, 0.0, 0.0, 0.0);
	playerLastpay[playerid] = gettime(); 
	
	SendClientMessageEx(playerid, COLOR_GREY, " คุณได้ทำการจ่ายเงินให้ %s จำนวน $%s.", ReturnName(playerb, 0), MoneyFormat(amount)); 
	SendClientMessageEx(playerb, COLOR_GREY, " คุณได้รับเงิน จำนวน $%s จาก %s", MoneyFormat(amount), ReturnName(playerid, 0)); 
...
CMD:isafk(playerid, params[])
{
	...
	if(GetTickCount() > (PlayerInfo[playerb][pPauseCheck]+2000))
		SendClientMessageEx(playerid, COLOR_GREY, "ผู้เล่น %s AFK %i วินาที!", ReturnName(playerb), PlayerInfo[playerb][pPauseTime]);
		
	else SendClientMessageEx(playerid, COLOR_GREY, "ผู้เล่น %s ไม่ได้ AFK", ReturnName(playerb)); 
...
CMD:id(playerid, params[])
{
	...
	if(inputID)
	{
		...
		if(!IsPlayerConnected(playerb))
			return SendClientMessage(playerid, COLOR_RED, "ผู้เล่นไม่ได้ทำการเชื่อมต่อเข้าเซืฟเวอร์");
	...
	else
	{
		...
		if(fullName)
		{
			foreach(new b : Player)
			{
				...
				else return SendClientMessage(playerid, COLOR_RED, "ไม่มีชื่อผู้เล่นนี้"); 
			}
		}
		else
		{
...
CMD:buy(playerid, params[])
{
	...
	if(!IsPlayerInBusiness(playerid))
		return SendErrorMessage(playerid, "คุณไม่ได้อยู่ในร้านค้า");
...
CMD:setslot(playerid, params[])
{
	...
	if(!PlayerInfo[playerid][pHasRadio])
		return SendUsageMessage(playerid, "คุณไม่มีวทิยุ");
		
	if(slot > 10 || slot < 1)
		return SendErrorMessage(playerid, "คุณสามารถเซ็ต สล็อตได้มากสูงสุดคือ (1-10)");
		
	PlayerInfo[playerid][pMainSlot] = slot;
		
	SendClientMessageEx(playerid, COLOR_YELLOWEX, "เปลี่ยน สล็อต วิทยุของคุณไปที่ %i.", slot); 
	SaveCharacter(playerid);
	return 1;
}
...
CMD:refill(playerid, params[])
{
	if(!PlayerInfo[playerid][pGascan])
		return SendErrorMessage(playerid, "คุณไม่มี กระติกน้ำมัน"); 
		
	if(IsPlayerInAnyVehicle(playerid))
		return SendErrorMessage(playerid, "คุณต้องออกมาเติมน้ำมัน นอกรถ");
		
	if(GetNearestVehicle(playerid) == INVALID_VEHICLE_ID)
		return SendErrorMessage(playerid, "คุณไม่ได้อยู่ใกล้รถ"); 
		
	if(playerRefillingVehicle[playerid])
		return SendErrorMessage(playerid, "คุณกำลังเติมน้ำมัน"); 
	...
	if(!IsPlayerInRangeOfPoint(playerid, 5.0, x, y, z))
		return SendErrorMessage(playerid, "คุณไม่ได้อยู่ฝาครอบน้ำมันรถ");
		
	if(VehicleInfo[vehicleid][eVehicleFuel] > 50)
		return SendErrorMessage(playerid, "น้ำมันคุณยังมากกว่า ครึ่งถัง"); 
		
	SendClientMessage(playerid, COLOR_ACTION, "ในระหว่างการเติมน้ำมันนั้น");
	SendClientMessage(playerid, COLOR_ACTION, "ถ้าคุณมีการขยับรถหรือการสตาทรถนั้นถือว่าเป็นการยกเลิกการเติมน้ำมันทันที"); 
...
CMD:respawnme(playerid, params[])
{
	if(GetPlayerTeam(playerid) != PLAYER_STATE_DEAD)
		return SendErrorMessage(playerid, "คุณยังไม่ได้เสียชีวิต");
		
	if(gettime() - PlayerInfo[playerid][pRespawnTime] < 60)
		return SendClientMessageEx(playerid, COLOR_YELLOWEX, "-> คุณจะสามารถยอมรับการตายในอีก %i วินาที จึงจะสามารถพิมพ์ /respawnme ได้", gettime() - PlayerInfo[playerid][pRespawnTime]); 
	...
	switch(id)
	{
		...
		{		
			...
			for(new c = 0; c < 3; c++)
			{
				...
				{
					format(str, sizeof(str), "%s บ้าน %i\n", str, c); 
				}
			}
			
			ShowPlayerDialog(playerid, DIALOG_SELECT_HOUSE, DIALOG_STYLE_LIST, "เลือกบ้าน:", str, "ตกลง", "<<"); 
		}
		case 3:
		{
			if(PlayerInfo[playerid][pSpawnPoint] == 2)
				return SendErrorMessage(playerid, "คุณได้ทำการเซ็ตจุดเกิดของคุณเป็น เฟคชั่นอยู่แล้ว");
...
CMD:sms(playerid, params[])
{
	...
	if(playerText[playerid])
		return SendClientMessage(playerid, COLOR_WHITE, "โปรดรอสักครู่"); 
		
	if(sscanf(params, "is[128]", phone_number, text))
		return SendUsageMessage(playerid, "/sms [หมายเลขโทรสัพท์] [ข้อความ]"); 
		
	foreach(new i : Player) if(PlayerInfo[i][pPhone] == phone_number){
		playerb = i;
	}
	
	SendClientMessage(playerid, COLOR_GREY, "[ ! ] คุณได้ทำการส่งข้อความ SMS ไปหาเบอร์ปลายทางเรียบร้อยแล้ว");

	format(str, sizeof(str), "* %s ได้พิมพ์บางอย่างลงไปในโทรสัพท์มือถือ", ReturnName(playerid, 0));
	SetPlayerChatBubble(playerid, str, COLOR_EMOTE, 20.0, 3000); 
...
CMD:buygun(playerid, params[])
{
	...
	if(!IsPlayerInBusiness(playerid))	
		return SendErrorMessage(playerid, "คุณไม่ได้อยู่ในร้านขายอาวุธ"); 
	...
	{
		SendUsageMessage(playerid, "/buygun [ชื่ออาวุธ] [กระสุน]"); 
		SendClientMessage(playerid, COLOR_GRAD2, "[ colt: $12,500; กระสุน: $25 ] [ deagle: $17,500; กระสุน: $38 ] [ shotgun: $17,500; กระสุน: $38 ]");
		SendClientMessage(playerid, COLOR_GRAD2, "[ rifle: $25,000; กระสุน: $100 ] [ parachute: $1,250; กระสุน: $1,250 ] [ armor: $2,000; กระสุน: $2,000 ]");
		return 1;
	}
	...
	if(!strcmp(a_str, "colt"))
	{
		if(sscanf(b_str, "i", ammo))
			return SendUsageMessage(playerid, "/buygun colt [กระสุน]"); 
			
		if(ammo < 1 || ammo > 32767)
			return SendErrorMessage(playerid, "กระสุน มากเกินไป");  
			
		totalPrice = ammo * 25 + 12500; 
		
		format(str, sizeof(str), "คุณมั่นใจแล้วใช่ไหมที่จะซื้อปืน 'Colt 45' ด้วยเงิน $%s?", MoneyFormat(totalPrice));
		ConfirmDialog(playerid, "Confirmation", str, "OnPlayerPurchaseWeapon", WEAPON_COLT45, ammo, totalPrice); 
	}
	else if(!strcmp(a_str, "deagle"))
	{
		if(sscanf(b_str, "i", ammo))
			return SendUsageMessage(playerid, "/buygun deagle [กระสุน]"); 
		...
		format(str, sizeof(str), "คุณมั่นใจแล้วใช่ไหมที่จะซื้อปืน 'Desert Eagle' ด้วยเงิน $%s?", MoneyFormat(totalPrice));
		ConfirmDialog(playerid, "Confirmation", str, "OnPlayerPurchaseWeapon", WEAPON_DEAGLE, ammo, totalPrice); 
	}
	else if(!strcmp(a_str, "shotgun"))
	{
		if(sscanf(b_str, "i", ammo))
			return SendUsageMessage(playerid, "/buygun shotgun [กระสุน]"); 
		...
		format(str, sizeof(str), "คุณมั่นใจแล้วใช่ไหมที่จะซื้อปืน 'Shotgun' ด้วยเงิน $%s?", MoneyFormat(totalPrice));
		ConfirmDialog(playerid, "Confirmation", str, "OnPlayerPurchaseWeapon", WEAPON_SHOTGUN, ammo, totalPrice); 
	}
	else if(!strcmp(a_str, "rifle"))
	{
		if(sscanf(b_str, "i", ammo))
			return SendUsageMessage(playerid, "/buygun rifle [กระสุน]"); 
		...
		format(str, sizeof(str), "คุณมั่นใจแล้วใช่ไหมที่จะซื้อปืน 'Rifle' ด้วยเงิน $%s?", MoneyFormat(totalPrice));
	...
	else if(!strcmp(a_str, "parachute"))
	{
		...
		format(str, sizeof(str), "คุณมั่นใจแล้วใช่ไหมที่จะซื้อ 'ร่มชูชีพ' ด้วยเงิน $%s?", MoneyFormat(totalPrice));
	...
	else if(!strcmp(a_str, "armor"))
	{
		...
		format(str, sizeof(str), "คุณมั่นใจแล้วใช่ไหมที่จะซื้อ 'เสื้อเกราะ' ด้วยเงิน $%s?", MoneyFormat(totalPrice));
...
CMD:buyammo(playerid, params[])
{
	...
	if(!IsPlayerInBusiness(playerid))	
		return SendErrorMessage(playerid, "คุณไม่ได้อยู่ร้านขายอาวุธ"); 
	...
	{
		SendUsageMessage(playerid, "/buyammo [ชื่ออาวุธ] [กระสุน]"); 
		SendClientMessage(playerid, COLOR_GRAD2, "[ colt: $25 ] [ deagle: $38 ] [ shotgun: $38 ] [ rifle: $100 ]");
		return 1; 
	}
	
	if(ammo < 1 || ammo > 32767)
			return SendErrorMessage(playerid, "กระสุนมากเกินไป");  
	
	if(!strcmp(weapon, "colt"))
	{
		if(!PlayerHasWeapon(playerid, 22))
			return SendErrorMessage(playerid, "คุณไม่ได้มีอาวุธชนิดนี้");
			
		totalPrice = 25 * ammo; 
		
		format(str, sizeof(str), "คุณมั่นใจแล้วใช่ไหมที่จะซื้อกระสุนในราคา $%s?", MoneyFormat(totalPrice)); 
...
CMD:unimpound(playerid, params[])
{
	if(!PlayerInfo[playerid][pVehicleSpawned] && !PlayerInfo[playerid][pAdminDuty])
		return SendErrorMessage(playerid, "รถของคุณไม่ได้ถูกนำออกมา");
	...
	if(VehicleInfo[vehicleid][eVehicleOwnerDBID] != PlayerInfo[playerid][pDBID])
		return SendErrorMessage(playerid, "คุณไม่ได้เป็นเจ้าของรถ");
		
	if(!VehicleInfo[vehicleid][eVehicleImpounded])
		return SendErrorMessage(playerid, "รถคุณไม่ได้ถูกยึด");
		
	if(5000 > PlayerInfo[playerid][pMoney])
		return SendErrorMessage(playerid, "คุณมีเงินไม่เพียงพอในการนำรถออกจากการยึดได้ (จำนวนเงินที่ต้องการ: $5,000)");
		
	VehicleInfo[vehicleid][eVehicleImpounded] = false;	
	GiveMoney(playerid, -1500);
	
	SendServerMessage(playerid, "%s ได้ถูกนำออกจากการยึด", ReturnVehicleName(vehicleid));
...
CMD:setstation(playerid, params[])
{
	if(!IsPlayerInAnyVehicle(playerid) && !IsPlayerInProperty(playerid))
		return SendErrorMessage(playerid, "คุณไม่ได้อยู่บนรถหรือภายในตัวบ้าน");
	...
	if(IsPlayerInAnyVehicle(playerid))
	{
		...
		if(GetPlayerVehicleSeat(playerid) > 1)
			return SendErrorMessage(playerid, "คุณไม่ได้อยู่ในที่นั่งคนขับหรือผู้โดยสารด้านหน้า"); 
			
		if(!VehicleInfo[vehicleid][eVehicleHasXMR])
			return SendClientMessage(playerid, COLOR_YELLOW, "รถคันนี้ไม่มี XM-Radio"); 
	}
	...
	if(!strcmp(params, "off"))
	{
		if(IsPlayerInAnyVehicle(playerid))
		{
			if(!VehicleInfo[vehicleid][eVehicleXMROn])
				return SendErrorMessage(playerid, "XM-Radio ไม่ได้ถูกเปิด"); 
	...
	if(!XMRStationInfo[id][eXMRStationID])
		return SendErrorMessage(playerid, "คุณระบุรหัสสถานีไม่ถูกต้อง"); 
...
{
	SendClientMessage(playerid, COLOR_RED, "คุณสามารถทิ้งอาวุธของคุณด้วยการพิมพ์ /leavegun [ไอดีอาวุธ]"); 
	
	for(new i = 0; i < 4; i++)
	{
		if(PlayerInfo[playerid][pWeaponsAmmo][i] > 0)
			SendClientMessageEx(playerid, COLOR_GREY, "[ID: %d] อาวุธ: [%s] - กระสุน: [%d]", PlayerInfo[playerid][pWeapons][i], ReturnWeaponName(PlayerInfo[playerid][pWeapons][i]), PlayerInfo[playerid][pWeaponsAmmo][i]);
	}
...
CMD:enter(playerid, params[])
{
	...
	SendServerMessage(playerid, "คุณไม่ได้อยู่หน้าทางเข้า"); 
	return 1;
}
...
CMD:exit(playerid, params[])
{
	...
	if(PlayerInfo[playerid][pEditingObject])
		return SendErrorMessage(playerid, "คุณยังอยู่ในโหมดการจัดการ Object");
	...
	SendServerMessage(playerid, "คุณไม่ได้อยู่ในบ้านหรือธุรกิจ");
	return 1;
}
...
CMD:vehicle(playerid, params[])
{
	...
	if(!strcmp(oneString, "get"))
	{
		...
		if(slotid < 1 || slotid > 5)
			return SendErrorMessage(playerid, "สล็อต มี 1-5เท่านั้น"); 
			
		if(!PlayerInfo[playerid][pOwnedVehicles][slotid])
			return SendErrorMessage(playerid, "คุณไม่มีรถใน สล็อตนี้"); 
			
		if(PlayerInfo[playerid][pVehicleSpawned] == true)
			return SendErrorMessage(playerid, "รถของคุณยังมีอยู่ภายในเซืฟเวอร์"); 
		...
		for(new i = 0; i < MAX_VEHICLES; i++)
		{
			if(VehicleInfo[i][eVehicleDBID] == PlayerInfo[playerid][pOwnedVehicles][slotid])
				return SendErrorMessage(playerid, "รถคันนี้ถูกนำออกมาแล้ว");
		}
	...
	else if(!strcmp(oneString, "buy"))
	{
		if(PlayerInfo[playerid][pVehicleSpawned])
			return SendErrorMessage(playerid, "รถของคุณยังไม่ถูดจัดเก็บในพื้นที่จอดรถ"); 
		...
		if(!idx)
		{
			SendServerMessage(playerid, "คุณไม่มีพื้นที่ในการซื้อรถคันใหม้แล้ว"); 
			return 1;
		}
	...
	else if(!strcmp(oneString, "scrap"))
	{
		...
		format(str, sizeof(str), "คุณมั่นใจแล้วใช่ไหมที่จะขายรถ %s?\nคุณจะได้เงินคืนในจำนวน : $%s\n\n{FF6347}คุณจะไม่สามารถนำรถคันนี้กลับมาใช้งานได้อีก", ReturnVehicleName(vehicleid), MoneyFormat(cash_back)); 
	...
	else if(!strcmp(oneString, "lock"))
	{
		...
		else SendServerMessage(playerid, "คุณไม่ได้อยู่ภายในรถหรือใกล้รถ");
	}
	...
	else if(!strcmp(oneString, "faction"))
	{
		...
		if(!PlayerInfo[playerid][pFaction]) return SendErrorMessage(playerid, "คุณไม่มีเฟคชั่น");
		
		if(PlayerInfo[playerid][pFactionRank] > FactionInfo[PlayerInfo[playerid][pFaction]][eFactionAlterRank])
			return SendClientMessage(playerid, COLOR_RED, "ACCESS DENIED:{FFFFFF} คุณไม่ใช่ผู้นำเฟคชั่น");
		...
		if(VehicleInfo[vehicleid][eVehicleFaction] != 0)
			return SendErrorMessage(playerid, "คุณได้นำรถของคุณเข้ากับเฟคชั่น %s แล้ว", ReturnFactionNameEx(VehicleInfo[vehicleid][eVehicleFaction]));
		
		VehicleInfo[vehicleid][eVehicleFaction] = PlayerInfo[playerid][pFaction]; 
		SendServerMessage(playerid, "รถคันนี้เป็นของเฟคชั่น %s.", ReturnFactionName(playerid));
	...
	{
		if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER)
			return SendErrorMessage(playerid, "คุณไม่ได้อยู่ภายในรถและเป็นคนัขบ");
		...
		SendServerMessage(playerid, "คุณได้นำรถ %s ออกจากเฟคชั่น %s", ReturnVehicleName(vehicleid), ReturnFactionNameEx(VehicleInfo[vehicleid][eVehicleFaction]));
	...
	else if(!strcmp(oneString, "list"))
	{
		if(CountPlayerVehicles(playerid) == 0)
			return SendServerMessage(playerid, "คุณไม่มีรถ");
			
		return ShowVehicleList(playerid); 
	}
	else return SendErrorMessage(playerid, "ผิดพลาด");
	return 1;
}
...
CMD:engine(playerid, params[])
{
	...
	if(HasNoEngine(vehicleid))
		return SendErrorMessage(playerid, "รถคันนี้ไม่สามารถ สตาร์ทได้"); 

	if(!VehicleInfo[vehicleid][eVehicleDBID] && !VehicleInfo[vehicleid][eVehicleAdminSpawn])
		return SendServerMessage(playerid, "รถคันนี้เป็นรถส่วนบุคคลที่ไม่สามารถให้ผู้เล่นธรรมดาใช้ได้");
		
	if(VehicleInfo[vehicleid][eVehicleFuel] < 1 && !VehicleInfo[vehicleid][eVehicleAdminSpawn])
		return SendClientMessage(playerid, COLOR_RED, "รถคันนี้น้ำมันหมด"); 
	
	if(VehicleInfo[vehicleid][eVehicleFaction] > 0)
	{
		if(PlayerInfo[playerid][pFaction] != VehicleInfo[vehicleid][eVehicleFaction] && !PlayerInfo[playerid][pAdminDuty])
		{
			return SendErrorMessage(playerid, "คุณไม่มีกุญแจ"); 
		}
	}
	...
	{
		SendNearbyMessage(playerid, 20.0, COLOR_EMOTE, "* %s ได้สตาร์ทเครื่องยนต์  %s.", ReturnName(playerid, 0), ReturnVehicleName(vehicleid)); 
		ToggleVehicleEngine(vehicleid, true); VehicleInfo[vehicleid][eVehicleEngineStatus] = true;
	}
	else
	{
		SendNearbyMessage(playerid, 20.0, COLOR_EMOTE, "* %s ได้ดับเครื่องยนต์ %s.", ReturnName(playerid, 0), ReturnVehicleName(vehicleid)); 
...
CMD:unscramble(playerid, params[])
{
	...
	if(!PlayerInfo[playerid][pUnscrambling])
		return SendErrorMessage(playerid, "คุณไม่ได้เดินสายไฟยานพาหนะ");
	...
	{ // This occurrs if they wrote the correct word:
		...
		if(PlayerInfo[playerid][pScrambleSuccess] >= 7)
		{
			...
			SendNearbyMessage(playerid, 20.0, COLOR_EMOTE, "* %s ได้สาตร์ทเครื่องยนต์ %s.", ReturnName(playerid, 0), ReturnVehicleName(vehicleid)); 
			ToggleVehicleEngine(vehicleid, true); VehicleInfo[vehicleid][eVehicleEngineStatus] = true;
		}	
	}
	else
...
CMD:check(playerid, params[])
{
	...
	if(!IsPlayerInAnyVehicle(playerid) && GetNearestVehicle(playerid) != INVALID_VEHICLE_ID)
	{
		...
		if(VehicleInfo[vehicleid][eVehicleFaction] && FactionInfo[VehicleInfo[vehicleid][eVehicleFaction]][eFactionType] != FACTION_TYPE_ILLEGAL && PlayerInfo[playerid][pFaction] != VehicleInfo[vehicleid][eVehicleFaction])
			return SendClientMessage(playerid, COLOR_YELLOW, "รถคันนี้เป็นรถของเฟคชั่นไม่สามารถใช้คำสั่ง /check ได้");
		...
		ShowPlayerDialog(playerid, DIALOG_VEHICLE_WEAPONS, DIALOG_STYLE_LIST, "Trunk:", principal_str, "ตกลง", "<<");
	}
	else if(IsPlayerInAnyVehicle(playerid))
	{
		new
	...
	else if(IsPlayerInProperty(playerid))
	{
		...
		ShowPlayerDialog(playerid, DIALOG_HOUSE_WEAPONS, DIALOG_STYLE_LIST, "Weapons:", longstr, "ตกลง", "ยกเลิก");		
	}
	return 1;
}
...
CMD:buyproperty(playerid, params[])
{
	...
	if(nearProperty)
	{
		if(PropertyInfo[id][ePropertyOwnerDBID])
			return SendErrorMessage(playerid, "คุณไม่ได้อยู่ใกล้หน้าบ้าน"); 
			
		if(PropertyInfo[id][ePropertyMarketPrice] > PlayerInfo[playerid][pMoney])
			return SendErrorMessage(playerid, "คุณมีเงินไม่เพียงพอในการซื้อบ้าน");
	...
	else return SendErrorMessage(playerid, "คุณไม่ได้อยู่ใกล้บ้าน");
	return 1; 
}
...
CMD:lock(playerid, params[])
{
	...
	if((id = IsPlayerNearProperty(playerid)) != 0)
	{
		if(PlayerInfo[playerid][pDBID] != PropertyInfo[id][ePropertyOwnerDBID])
			return SendErrorMessage(playerid, "คุณไม่มีกุญแจบ้าน"); 
	...
	{
		if(PlayerInfo[playerid][pDBID] != PropertyInfo[id][ePropertyOwnerDBID])
			return SendErrorMessage(playerid, "คุณไม่ได้มีกุญแจบ้าน"); 
			
		if(!IsPlayerInRangeOfPoint(playerid, 3.0, PropertyInfo[id][ePropertyInterior][0], PropertyInfo[id][ePropertyInterior][1], PropertyInfo[id][ePropertyInterior][2]))
			return SendErrorMessage(playerid, "คุณไม่ได้อยู่ใกล้ประตูบ้าน"); 
	...
	if((b_id = IsPlayerNearBusiness(playerid)) != 0)
	{
		if(BusinessInfo[b_id][eBusinessOwnerDBID] != PlayerInfo[playerid][pDBID])
			return SendErrorMessage(playerid, "คุณไม่ได้อยู่ใกล้ประตูกิจการ"); 
	...
	else if((b_id = IsPlayerInBusiness(playerid)) != 0)
	{
		...
		if(BusinessInfo[b_id][eBusinessOwnerDBID] != PlayerInfo[playerid][pDBID])
			return SendErrorMessage(playerid, "คุณไม่มีกุญแจกิจการ"); 
...
CMD:placepos(playerid, params[])
{
	...
	if((id = IsPlayerInProperty(playerid)) != 0)
	{
		...
		SendServerMessage(playerid, "คุณได้ตั้งจุดตู้เซฟของบ้านคุณไว้ที่ตรงนี้"); 
	}
	else return SendErrorMessage(playerid, "คุณไม่ได้อยู่ภายในบ้าน");
...
CMD:property(playerid, params[])
{
	...
	if((id = IsPlayerInProperty(playerid)) == 0)
		return SendErrorMessage(playerid, "คุณไม่ดูภายในบ้าน");
	...
	if(!strcmp(str, "info"))
	{
		...
		SendClientMessageEx(playerid, COLOR_WHITE, "บ้าน ไอดี:[%i], ราคาขาย:[$%s], เลเวล:[%i], ประเภท:[%s]", PropertyInfo[id][ePropertyDBID], MoneyFormat(PropertyInfo[id][ePropertyMarketPrice]), PropertyInfo[id][ePropertyLevel], type); 
	}
	else if(!strcmp(str, "cashbox"))
	{
		...
		if(!strcmp(pick, "info"))
		{
			SendClientMessageEx(playerid, COLOR_ACTION, "คุณมีเงินอยู่ในตู้เซฟภายในบ้าน จำนวน $%s", MoneyFormat(PropertyInfo[id][ePropertyCashbox])); 
		}
		else if(!strcmp(pick, "place"))
		{	
			if(sscanf(cstr, "i", amount))
				return SendUsageMessage(playerid, "/property cashbox place [จำนวน]"); 
			...
			SendServerMessage(playerid, "คุณได้เก็บเงินบางส่วนไว้ในตู้เซฟถายในบ้านของคุณ", amount); 
			SaveProperty(id);
		}
		else if(!strcmp(pick, "take"))
		{
			if(sscanf(cstr, "i", amount))
				return SendUsageMessage(playerid, "/property cashbox take [จำนวน]");
				
			if(amount > PropertyInfo[id][ePropertyCashbox])
				return SendErrorMessage(playerid, "เงินในตู้เซฟภายในบ้านของคุณมีไม่เพียงพอตามจำนวนที่คุณต้องการ");
				
			GiveMoney(playerid, amount);
			PropertyInfo[id][ePropertyCashbox]-= amount;
			
			SendServerMessage(playerid, "คุณได้หยิบเงินออกจากตู้เซฟของคุณมาจำนวน $%s", amount); 
			SaveProperty(id); 
		}
	...
	else if(!strcmp(str, "boombox"))
	{
		...
		if(!strcmp(pick, "add"))
		{
			if(PropertyInfo[id][ePropertyHasBoombox])
				return SendErrorMessage(playerid, "บ้านหลังนี้มี 'BoomBox' อยู่แล้ว"); 
				
			if(PlayerInfo[playerid][pEditingObject])
				return SendErrorMessage(playerid, "คุณกำลังอยู่ในโหมดแก้ไข 'Object' อยู่ในขณะนี้ "); 
			...
			SendServerMessage(playerid, "วาง 'Boombox' ในต่ำแหน่งที่คุณคิดว่าโอเครแล้ว กดปุ่มบันทึกเพื่อทำการซื้อด้วยเงินในราคา $1,000"); 
			PlayerInfo[playerid][pEditingObject] = 1;
		}
		else if(!strcmp(pick, "remove"))
		{
			if(!PropertyInfo[id][ePropertyHasBoombox])
				return SendErrorMessage(playerid, "บ้านหลังนี้ไม่มี 'Boombox'"); 
				
			ConfirmDialog(playerid, "Confirmation", "คุณมั่นใจแล้วใช่ไหมว่าคุณจะลบ 'boombox' ออกจากบ้านของคุณ?", "OnRemoveBoombox", id);
		}
		else if(!strcmp(pick, "adjust"))
		{
			...
			SendServerMessage(playerid, "คุณได้ปรับต่ำแหน่งของ 'BoomBox' ของคุณเรียบร้อยแล้ว ");
		}
	}
	else return SendErrorMessage(playerid, "ผิดพลาด"); 
...
{
	if (isnull(params)) 
		return SendUsageMessage(playerid, "/me [การกระทำ]");
...
{
	if (isnull(params)) 
		return SendUsageMessage(playerid, "/do [กระกระทำ]");
...
{
	if (isnull(params))
		return SendUsageMessage(playerid, "/ame [การกระทำ]");
...
{
	if (isnull(params)) 
		return SendUsageMessage(playerid, "/my [การกระทำ]");
...
{
	if (isnull(params))
		return SendUsageMessage(playerid, "/amy [การกระทำ]");
...
{
	if (isnull(params))
		return SendUsageMessage(playerid, "/shout [ข้อความ]"); 
		
	if(GetPlayerTeam(playerid) != PLAYER_STATE_ALIVE)
		return SendErrorMessage(playerid, "คุณไม่สามารถตะโกนได้");
	...
	if(isCaps == true)
	{
		if(strlen(params) > 84)
		{
			SendNearbyMessage(playerid, 30.0, COLOR_WHITE, "%s ตะโกน: %.84s!", ReturnName(playerid, 0), params);
			SendNearbyMessage(playerid, 30.0, COLOR_WHITE, "%s ตะโกน: ...%s!", params[84]);
		}
		else SendNearbyMessage(playerid, 30.0, COLOR_WHITE, "%s ตะโกน: %s!", ReturnName(playerid, 0), params);
	}
	else
	{
		if(strlen(params) > 84)
		{
			SendNearbyMessage(playerid, 20.0, COLOR_WHITE, "%s ตะโกน: %.84s!", ReturnName(playerid, 0), params);
			SendNearbyMessage(playerid, 20.0, COLOR_WHITE, "%s ตะโกน: ...%s!", params[84]);
		}
		else SendNearbyMessage(playerid, 20.0, COLOR_WHITE, "%s ตะโกน: %s!", ReturnName(playerid, 0), params);
	}
	return 1;
}
...
{
	if(GetPlayerTeam(playerid) == PLAYER_STATE_DEAD)
		return SendErrorMessage(playerid, "คุณไม่สามารถพูดในขณะที่คุณไม่มีชีวิต");
	
	if(isnull(params))
		return SendUsageMessage(playerid, "/low [ข้อความ]");
	...
	if(strlen(params) > 84)
	{
		format(str, sizeof(str), "%s พูดว่า[เบา]: %.84s", ReturnName(playerid, 0), params);
		LocalChat(playerid, 6.0, str, COLOR_FADE5, COLOR_FADE4, COLOR_FADE3, COLOR_FADE3); 
		
		format(str, sizeof(str), "%s พูดว่า[เบา]: ... %s", ReturnName(playerid, 0), params[84]); 
		LocalChat(playerid, 6.0, str, COLOR_FADE5, COLOR_FADE4, COLOR_FADE3, COLOR_FADE3); 
	}
	else
	{
		format(str, sizeof(str), "%s พูดว่า[เบา]: %s", ReturnName(playerid, 0), params); 
		LocalChat(playerid, 6.0, str, COLOR_FADE5, COLOR_FADE4, COLOR_FADE3, COLOR_FADE3); 
	}
...
CMD:buybiz(playerid, params[])
{
	...
	if((id = IsPlayerNearBusiness(playerid)) != 0)
	{
		if(CountPlayerBusiness(playerid) == 1)
			return SendErrorMessage(playerid, "คุณสามารถกิจการไดีแค่ 1 กิจการเท่านั้น"); 
			
		if(BusinessInfo[id][eBusinessOwnerDBID])
			return SendErrorMessage(playerid, "คุณเป็นเจ้าของกิจการนี้อยู่แล้ว"); 
			
		if(PlayerInfo[playerid][pLevel] < BusinessInfo[id][eBusinessLevel])
			return SendErrorMessage(playerid, "คุณยังมีเลเวลไม่เพียงพอ จำเป็นต้องมีเลเวล %i ก่อนจึงจะสามารถซื้อได้", BusinessInfo[id][eBusinessLevel]);
			
		if(PlayerInfo[playerid][pMoney] < BusinessInfo[id][eBusinessMarketPrice])
			return SendErrorMessage(playerid, "คุณมีเงินไม่เพียงพอในการซื้อกิจการ"); 
			
		PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
		GameTextForPlayer(playerid, "Congratulations!~n~You're now the owner of this business!", 4000, 5);
		
		SendClientMessageEx(playerid, COLOR_ACTION, "คุณได้ซื้อกิจการ %s ด้วยเงิน $%s!", BusinessInfo[id][eBusinessName], MoneyFormat(BusinessInfo[id][eBusinessMarketPrice]));
	...
	else return SendErrorMessage(playerid, "คุณไม่ได้อยู่ใกล้กิจการ");
...
CMD:bizinfo(playerid, params[])
{
	...
	else return SendErrorMessage(playerid, "คุณไม่ได้อยู่ในกิจการ");
...
CMD:bizfee(playerid, params[])
{
	...
	if((id = IsPlayerInBusiness(playerid)) != 0)
	{
		...
		if(sscanf(params, "i", amount))
			return SendUsageMessage(playerid, "/bizfee [จำนวน]"); 
			
		if(amount > 1500)
			return SendErrorMessage(playerid, "ค่าทางเข้าต้องน้อยกว่า $1,500.");
			
		SendServerMessage(playerid, "คุณได้เซ็ตค่าทางเข้าเป็นจำนวนเงิน $%s.", MoneyFormat(amount));
...
CMD:bizcash(playerid, params[])
{
	...
	if((id = IsPlayerInBusiness(playerid)) != 0)
	{
		...
		{
			SendClientMessageEx(playerid, COLOR_ACTION, "เงินในกิจการของคุณมี $%s", MoneyFormat(BusinessInfo[id][eBusinessCashbox])); 
		}
		else if(!strcmp(astr, "deposit"))
		{	
			if(sscanf(bstr, "i", amount))
				return SendUsageMessage(playerid, "/bizcash deposit [จำนวน]"); 
				
			if(amount > PlayerInfo[playerid][pMoney])
				return SendErrorMessage(playerid, "คุณมีเงินไมเพียงพอในการฝากเงินไว้ในกิจการ");
				
			BusinessInfo[id][eBusinessCashbox]+= amount;
			GiveMoney(playerid, -amount);
			
			SendClientMessageEx(playerid, COLOR_ACTION, "คุณได้ฝากเงินกับกิจการของคุณจำนวน $%s (รวม: $%s)", MoneyFormat(amount), MoneyFormat(BusinessInfo[id][eBusinessCashbox]));
			SaveBusiness(id); SaveCharacter(playerid);
		}
		...
		{	
			...
			if(amount > BusinessInfo[id][eBusinessCashbox])
				return SendErrorMessage(playerid, "เงินในกิจการของคุณไม่เพียงพอต่อจำนวนที่คุณต้องการ");
				
			BusinessInfo[id][eBusinessCashbox] -= amount; 
			GiveMoney(playerid, amount); 
			
			SendClientMessageEx(playerid, COLOR_ACTION, "คุณได้ถอนเงินออกจากกิจการของคุณจำนวน $%s (รวม: $%s)", MoneyFormat(amount), MoneyFormat(BusinessInfo[id][eBusinessCashbox]));
			SaveBusiness(id); SaveCharacter(playerid);
		}
		else return SendErrorMessage(playerid, "ผิดพลาด");
	}
...
CMD:sellbiz(playerid, params[])
{
	...
	if((id = IsPlayerInBusiness(playerid)) != 0)
	{
		...
		format(str, sizeof(str), "คุณแน่ใจแล้วใช่ไหมที่คุณจะขายกิจการของคุณทิ้ง?\nคุณจะได้รับเงินคืน $%s และเงินในกิจการของคุณ $%s", MoneyFormat(BusinessInfo[id][eBusinessMarketPrice] / 2), MoneyFormat(BusinessInfo[id][eBusinessCashbox]));
...
CMD:factionhelp(playerid, params[])
{
	...
	if(ReturnFactionType(playerid) == FACTION_TYPE_GOVERMENT)
	{
...
CMD:f(playerid, params[])
{
	...
	if(PlayerInfo[playerid][pFactionRank] > FactionInfo[PlayerInfo[playerid][pFaction]][eFactionChatRank])
		return SendErrorMessage(playerid, "ยศ/ต่ำแหน่งของคุณ ไม่ได้รับอนุญาติให้พูดคุยในแชทเฟคชั่นได้");
...
CMD:ouninvite(playerid, params[])
{
	...
	if(isnull(params))
		return SendUsageMessage(playerid, "/ouninvite [ชื่อจริง_นามกสุล]"); 
		
	foreach(new i : Player)
	{
		if(!strcmp(ReturnName(i), params))
		{
			SendServerMessage(playerid, "%s ผู้เล่นได้กลับมาออกไลน์แล้ว (ID: %i)", ReturnName(i), i);
			return 1;
		}
	}
	...
	if(!cache_num_rows())
	{
		SendServerMessage(playerid, "%s ไม่พบข้อมูลในฐานข้อมูล", params);
		cache_delete(cache);
		return 1;
	}
	...
	if(playerFaction != PlayerInfo[playerid][pFaction])
		return SendErrorMessage(playerid, "%s ไม่ได้อยู่ในเฟคชั่นของคุณ", params); 
		
	if(playerRank > PlayerInfo[playerid][pFactionRank])
		return SendErrorMessage(playerid, "%s ยศ/ต่ำแหน่ง สูงกว่าคุณ", params);
	...
	SendServerMessage(playerid, "%s ได้ออกจากเฟคชั่นเรียบร้อยแล้ว", params);
	return 1;
}
...
CMD:rank(playerid, params[])
{
	...
	if(sscanf(params, "ui", playerb, rank))
	{
		...
		SendUsageMessage(playerid, "/rank [ชื่อบางส่วน/ไอดี] [ต่ำแหน่ง/ยศ]");
		return 1;
	}
	...
	SendClientMessageEx(playerb, COLOR_YELLOW, "ยศ/ต่ำแหน่ง ของคุณได้มีการอัปเกรดจาก %s เป็น %s", FactionRanks[PlayerInfo[playerb][pFaction]][PlayerInfo[playerb][pFactionRank]], FactionRanks[PlayerInfo[playerb][pFaction]][rank], ReturnName(playerid, 0)); 
	SendClientMessageEx(playerid, COLOR_YELLOW, "คุณได้อัปเกรด ยศ/ต่ำแหน่งของ %s จาก %s เป็น %s!", ReturnName(playerb, 0), FactionRanks[PlayerInfo[playerb][pFaction]][PlayerInfo[playerb][pFactionRank]], FactionRanks[PlayerInfo[playerb][pFaction]][rank]);
...
CMD:handcuff(playerid, params[])
{
	...
	if(sscanf(params, "u", playerb))
		return SendUsageMessage(playerid, "/handcuff [ชื่อบางส่วน/ไอดี]"); 
	...
	SendServerMessage(playerb, "คุณถูก %s รวบแขนและใส่กุญแจมือ", ReturnName(playerid, 0)); 
	SendServerMessage(playerid, "คุณได้รวบแขนของ %s และใส่กุญแจมือ", ReturnName(playerb, 0));
	return 1;
}
...
CMD:unhandcuff(playerid, params[])
{
	...
	if(sscanf(params, "u", playerb))
		return SendUsageMessage(playerid, "/unhandcuff [ชื่อบางส่วน/ไอดี]"); 
	...
	SendServerMessage(playerid, "ปลดกุญแจมือของ %s", ReturnName(playerb, 0));
	return 1;
}
...
CMD:uniform(playerid, params[])
{
	...
	if(FactionInfo[PlayerInfo[playerid][pFaction]][eFactionType] != FACTION_TYPE_GOVERMENT && FactionInfo[PlayerInfo[playerid][pFaction]][eFactionType] != FACTION_TYPE_GOVERMENT)
		return SendClientMessage(playerid, COLOR_RED, "ACCESS DENIED:{FFFFFF} คุณไม่ใช่ ตำรวจ/นายอำเภอ/ข้าราชการเรือนจำ"); 
...
CMD:megaphone(playerid, params[])
{
	...
	SendNearbyMessage(playerid, 40.0, COLOR_YELLOWEX, "[%s:o< %s ]", ReturnName(playerid, 0), params);
	return 1;
}
...
CMD:department(playerid, params[])
{
	...
	if(isnull(params))
		return SendUsageMessage(playerid, "/department [ข้อความ]");
	...
	foreach(new i : Player)
	{
		...
		if(FactionInfo[factionid][eFactionType] == FACTION_TYPE_GOVERMENT)
		{
...
CMD:taser(playerid, params[])
{
	...
	if(!PlayerInfo[playerid][pPoliceDuty])
		return SendClientMessage(playerid, COLOR_RED, "ACCESS DENIED:{FFFFFF} คุณไม่ได้ปฏิบัตรงาน"); 
	...
	else
	{
		...
		SendNearbyMessage(playerid, 20.0, COLOR_EMOTE, "* %s เก็ยปืนช็อตไฟฟ้าเข้าซองหนัง", ReturnName(playerid, 0)); 
	}
	return 1; 
}
...
CMD:towcars(playerid, params[])
{
	...
	if(PlayerInfo[playerid][pAdminDuty])
	{
		if(sscanf(params, "i", factionid))
			return SendUsageMessage(playerid, "/towcars [เฟคชั่น ไอดี]"); 
			
		if(!FactionInfo[factionid][eFactionDBID] || factionid > MAX_FACTIONS)
			return SendErrorMessage(playerid, "เฟคชั่นไอดีที่คุณต้องการไม่มีอยู่ในฐานข้อมูล");
		...
		if(vehicle_found)
		{
			foreach(new g : Player) if(PlayerInfo[g][pFaction] == factionid)
				SendFactionMessageEx(playerid, COLOR_RED, "(( Administrator %s ได้ส่งรถที่ไม่ได้มีคนนั่งกลับจุดเกิดทั้งหมด ))", ReturnName(playerid));
		}
		else SendErrorMessage(playerid, "ไม่มีรถอยู่ในเฟคชั่น");
		return 1;
	}
	...
	if(vehicle_found)
		SendFactionMessageEx(playerid, COLOR_RED, "(( %s ได้ส่งรถที่ไม่ได้มีคนนั่งกลับจุดเกิดทั้งหมด ))", ReturnName(playerid));
...
CMD:take(playerid, params[])
{
	...
	{
		SendUsageMessage(playerid, "/take [ชื่อบางส่วน/ไอดี] [ไอเท็ม]");
	...
	if(!strcmp(a_str, "driverlicense"))
	{
		if(!PlayerInfo[playerb][pDriversLicense])
			return SendErrorMessage(playerid, "%s ไม่มีใบอนุญาติขับขี่", ReturnName(playerb, 0)); 
			
		PlayerInfo[playerb][pDriversLicense] = 0;
		SendNearbyMessage(playerid, 20.0, COLOR_EMOTE, "*%s ยึดใบอนุญาติขับขี่จาก %s", ReturnName(playerid, 0), ReturnName(playerb, 0));
	}
	else if(!strcmp(a_str, "weaponlicense"))
	{
		if(!PlayerInfo[playerb][pWeaponsLicense])
			return SendErrorMessage(playerid, "%s ไม่มีใบอนุญาติพออาวุธ", ReturnName(playerb, 0)); 
			
		PlayerInfo[playerb][pWeaponsLicense] = 0;
		SendNearbyMessage(playerid, 20.0, COLOR_EMOTE, "*%s ยึดใบอนุญาติพกอาวุธจาก %s", ReturnName(playerid, 0), ReturnName(playerb, 0));
	}
	else if(!strcmp(a_str, "weapons"))
	{
		TakePlayerGuns(playerb);
		SendNearbyMessage(playerid, 20.0, COLOR_EMOTE, "*%s ยึดอาวุธทั้งหมดในตัวของ %s", ReturnName(playerid, 0), ReturnName(playerb, 0));
		return 1;
	}
	else return SendServerMessage(playerid, "ผิดพลาด");
	return 1;
}
...
CMD:givelicense(playerid, params[])
{
	...
	if(PlayerInfo[playerid][pFactionRank] >= FactionInfo[PlayerInfo[playerid][pFaction]][eFactionAlterRank])
		return SendServerMessage(playerid, "ยศ/ต่ำแหน่งของคุณ ไม่ได้รับอนุญาติให้ใช้คำสั่งนี้"); 
	...
	if (sscanf(params, "u", playerb))
	{
		SendUsageMessage(playerid, "/givelicense [ชื่อบางส่วน/ไอดี]");
		SendServerMessage(playerid, "ผู้เล่นที่ได้รับใบอนุญาติพกอาวุธนั้นจะสามารถซื้ออาวุธในร้านปืนได้"); 
		return 1;
	}
	...
	if(!PlayerInfo[playerid][pWeaponsLicense]) {
		...
		SendPoliceMessage(COLOR_COP, "** HQ: %s %s ได้มอบใบ อนุญาติการพกอาวุธ ให้กับ %s*", ReturnFactionRank(playerid), ReturnName(playerid, 0), ReturnName(playerb)); 
	}
	else {
		PlayerInfo[playerid][pWeaponsLicense] = 0; 
		
		SendPoliceMessage(COLOR_COP, "** HQ: %s %s ได้ลบใบ อนุญาติการพกอาวุธ ให้กับ %s **", ReturnFactionRank(playerid), ReturnName(playerid, 0), ReturnName(playerb)); 
	}
...
CMD:impound(playerid, params[])
{
	...
	if(VehicleInfo[vehicleid][eVehicleFaction])
		return SendErrorMessage(playerid, "รถคันนี้เป็นรถของเฟคชั่น"); 
		
	if(!IsPlayerInDynamicArea(playerid, ImpoundLotArea))
		return SendErrorMessage(playerid, "คุณไม่ได้อยู่ในจุดยึดรถ"); 
	...
	SendClientMessageEx(playerid, COLOR_DARKGREEN, "คุณได้ยึดรถของ %s รุ่น %s เรียบร้อยแล้ว", ReturnDBIDName(VehicleInfo[vehicleid][eVehicleOwnerDBID]), ReturnVehicleName(vehicleid)); 
	
	foreach(new i : Player) if(PlayerInfo[i][pDBID] == VehicleInfo[vehicleid][eVehicleOwnerDBID])
		SendClientMessageEx(i, COLOR_DARKGREEN, "รถ %s ของคุณได้ถูก เจ้าหน้าที่ %s ยึดเรียบร้อยแล้ว", ReturnVehicleName(vehicleid), ReturnName(playerid)); 
...
CMD:wanted(playerid, params[])
{
	...
	if(sscanf(params, "us[90]", playerb, charge))
		return SendUsageMessage(playerid, "/wanted [ชื่อบางส่วน/ไอดี] [reason]"); 
...
CMD:aduty(playerid, params[])
{
	...
	if(PlayerInfo[playerid][pAdminDuty])
	{
		...
		format(str, sizeof(str), "%s ได้ออกจากการปฏิบัตรหน้าที่เป็นผู้ดูแลระบบแล้วในขณะนี้", ReturnName(playerid)); 
		SendAdminMessage(1, str);
	...
	else
	{
		...
		format(str, sizeof(str), "%s ได้เริ่มปฏิบัตรหน้าที่เป็นผู้ดูแลระบบแล้วในขณะนี้", ReturnName(playerid)); 
		SendAdminMessage(1, str);
...
CMD:gethere(playerid, params[])
{
	...
	SendServerMessage(playerb, "คุณถูกเคลื่อนย้ายโดยผู้ดูแลระบบ  %s", ReturnName(playerb));
...
CMD:showmain(playerid, params[])
{
	...
	SendServerMessage(playerid, "%s' UCP \"%s\" (DBID: %i).", ReturnName(playerb), e_pAccountData[playerb][mAccName], e_pAccountData[playerb][mDBID]);	
	return 1;
}
...
CMD:kick(playerid, params[])
{
	...
	if(PlayerInfo[playerb][pAdmin] > PlayerInfo[playerid][pAdmin])
		return SendErrorMessage(playerid, "คุณไม่ได้สามารถเตะ ผู้ดูแลระบบที่มีต่ำแหน่งสูงกว่าคุณได้", ReturnName(playerb)); 
		
	if(strlen(reason) > 56)
	{
		SendClientMessageToAllEx(COLOR_RED, "AdmCmd: %s ถูกเตะออกจากเซืฟเวอร์โดย %s สาเหตุ: %.56s", ReturnName(playerb), ReturnName(playerid), reason);
		SendClientMessageToAllEx(COLOR_RED, "AdmCmd: ...%s", reason[56]); 
	}
	else SendClientMessageToAllEx(COLOR_RED, "AdmCmd: %s ถูกเตะออกจากเซืฟเวอร์โดย %s สาเหตุ: %s", ReturnName(playerb), ReturnName(playerid), reason);
	
	new insertLog[256];
	
	if(e_pAccountData[playerb][mLoggedin] == false)
	{
		SendServerMessage(playerid, "ผู้เล่น (%s) ถูกเตะออกจากเซืฟเวอร์ขณะเข้าสู่ระบบ", ReturnName(playerb));
	}
...
CMD:oban(playerid, params[])
{
	...
	if(sscanf(params, "s[32]s[120]", playerb, reason))
		return SendUsageMessage(playerid, "/offlineban [ชื่อเต็ม] [สาเหตุ]");
		
	foreach(new i : Player)
	{
		if(!strcmp(ReturnName(i), playerb))
		{
			SendServerMessage(playerid, "%s ได้เชื่อมต่อเข้ามาในเซืฟเวอร์แล้ว (ID: %i)", playerb, i);
			return 1;
		}
	}
	
	if(!DoesPlayerExist(playerb))
		return SendErrorMessage(playerid, "%s ไม่พบเจอในฐานข้อมูล", playerb); 
...
CMD:oajail(playerid, params[])
{
	...
	if(sscanf(params, "s[32]ds[128]", playerb, length, reason))
		return SendUsageMessage(playerid, "/offlineajail [ชื่อเต็ม] [เวลา] [สาเหตุ]"); 
		
	foreach(new i : Player)
	{
		if(!strcmp(ReturnName(i), playerb))
		{
			SendServerMessage(playerid, "%s ได้เชื่อมต่อเข้ากับเซืฟเวอร์แล้ว (ID: %i)", playerb, i);
			return 1;
		}
	}
	
	if(!DoesPlayerExist(playerb))
		return SendErrorMessage(playerid, "ไม่พบชื่อ %s อยู่ในฐานข้อมูล", playerb); 
...
CMD:skin(playerid, params[])
{
	...
	if (sscanf(params, "ud", playerb, skinid)) 
		return SendUsageMessage(playerid, "/skin [ชื่อบางส่วน/ไอดี] [skinid]"); 
...
CMD:health(playerid, params[])
{
	...
	if (sscanf(params, "ud", playerb, health)) 
		return SendUsageMessage(playerid, "/health [ชื่อบางส่วน/ไอดี] [health]"); 
...
CMD:mute(playerid, params[])
{
	...
	if (sscanf(params, "u", playerb))
		return SendUsageMessage(playerid, "/mute [ชื่อบางส่วน/ไอดี]");
	...
	if(PlayerInfo[playerb][pMuted] == false)
	{
		...
		SendClientMessageToAllEx(COLOR_RED, "%s ใบ้ %s.", ReturnName(playerid), ReturnName(playerb)); 
	}
	else
	{
		PlayerInfo[playerb][pMuted] = false;
		SendClientMessageToAllEx(COLOR_RED, "%s ยกเลิกใบ้ %s.", ReturnName(playerid), ReturnName(playerb));
	}
	return 1;
}
...
CMD:thaw(playerid, params[])
{
	...
	if (sscanf(params, "u", playerb))
		return SendUsageMessage(playerid, "/thaw [ชื่อบางส่วน/ไอดี]");
...
CMD:awp(playerid, params[])
{
	...
	if (sscanf(params, "u", playerb))
		return SendUsageMessage(playerid, "/awp [ชื่อบางส่วน/ไอดี]");
	...
	SendServerMessage(playerid, "ตอนนี้คุณกำลังส่องผู้เล่น %s  /watchoff เพื่ออยุดพิมพ์", ReturnName(playerb));
	return 1;
}
...
CMD:arecord(playerid, params[])
{
	...
	if(sscanf(params, "s[60]s[30]", playerb, type))
		return SendUsageMessage(playerid, "/arecord [ชื่อเต็ม] [ajail, kicks, bans]");
		
	if(!ReturnDBIDFromName(playerb))
		return SendErrorMessage(playerid, "ไม่พบอยู่ในฐานข้อมูล");
...
CMD:armor(playerid, params[])
{
	...
	if (sscanf(params, "ud", playerb, armor)) 
		return SendUsageMessage(playerid, "/armor [ชื่อบางส่วน/ไอดี] [armor]"); 
	...
	if(armor > 200)
		return SendErrorMessage(playerid, "เซ็ตเกราะได้ไม่เกิน 200"); 
...
CMD:clearreports(playerid, params[])
{
	...
	if(reportCount)
	{
		...
		format(string, sizeof(string), "{FFFFFF}คุณมันใจหรืป่าวที่จะลบการรายงานทั้งหมด?\n\nมีรายงานทั้งหมด {FF6347}%d{FFFFFF}", reportCount);
...
CMD:p2p(playerid, params[])
{
	...
	if(sscanf(params, "uu", playerb, targetid))
		return SendUsageMessage(playerid, "/p2p [ผู้เล่น] [ผู้เล่นที่ต้องการให้ไปหา]");
	
	if (!IsPlayerConnected(playerb) || !IsPlayerConnected(targetid))
		return SendErrorMessage(playerid, "ผู้เล่นไม่ได้มีการเชื่อมต่อกับเซืฟเอวร์"); 
		
	if (e_pAccountData[playerb][mLoggedin] == false || e_pAccountData[targetid][mLoggedin] == false)
		return SendErrorMessage(playerid, "ผู้เล่นกำลังเข้าสู่ระบบ"); 
		
	format(str, sizeof(str), "%s ได้เคลื่อนย้าย %s ไปหา %s", ReturnName(playerid), ReturnName(playerb), ReturnName(targetid));
	SendAdminMessage(1, str);
...
CMD:gotoproperty(playerid, params[])
{
	...
	if(sscanf(params, "i", id))
		return SendUsageMessage(playerid, "/gotoproperty [บ้าน ไอดี]");
	...
	SendServerMessage(playerid, "คุณได้เคลื่อนย้ายบ้านไปในไอดี %i.", id);
...
CMD:gotobusiness(playerid, params[])
{
	...
	if(sscanf(params, "i", id))
		return SendUsageMessage(playerid, "/gotobusiness [กิจการไอดี]");
		
	if(!BusinessInfo[id][eBusinessDBID] || id > MAX_BUSINESS)
		return SendErrorMessage(playerid, "ไม่มีกิจการไอดีที่ต้องการ");
	...
	SendServerMessage(playerid, "คุณได้เคลื่อยย้ายไปหน้าทางเข้ากิจการ %i.", id); 
...
CMD:gotofaction(playerid, params[])
{
	...
	if(sscanf(params, "i", id))
		return SendUsageMessage(playerid, "/gotofaction [เฟคชั่นไอดี]");
	...
	SendServerMessage(playerid, "คุณได้เคลื่อยย้ายไปเฟคชั่น %s", ReturnFactionNameEx(id)); 
...
CMD:backup(playerid, params[])
{
	...
	if(!strcmp(params, "players"))
	{
		...
		format(str, sizeof(str), "%s เซฟข้อมูลผู้เล่นทั้งหมด (%i)", ReturnName(playerid), GetPlayerPoolSize());
		SendAdminMessage(1, str); 
	}
	else if(!strcmp(params, "all"))
	{
		...
		SendClientMessageToAllEx(COLOR_RED, "ผู้ดูแลระบบ %s ได้เซฟข้อมูลทั้งหมดลงในฐานข้อมูล", ReturnName(playerid));
	}
	else return SendErrorMessage(playerid, "ผิดพลาด");	
	return 1;
}
...
CMD:repair(playerid, params[])
{
	...
	format(str, sizeof(str), "%s ซ่อมรถไอดี %i.", ReturnName(playerid), vehicleid);
	SendAdminMessage(1, str);
...
CMD:spawncar(playerid, params[])
{
	...
	if(sscanf(params, "iiiI(0)", modelid, color1, color2, siren))
	{
		...
		SendServerMessage(playerid, "การเสดรถจะเป็นรถชั่วคราวไม่สามารถนำไปใช้งานแบบเต็มรูปแบบได้"); 
		return 1;
	}
	
	if(gettime() - lastVehicleSpawn[playerid] < 5)
		return SendServerMessage(playerid, "โปรดรอเวลาอีกสักครู่จึงค่อยเสกรถใหม่");
	
	if(modelid < 400 || modelid > 611)
		return SendErrorMessage(playerid, "ไม่มีรถไอดีที่ต้องการ");
		
	if(color1 < 0 || color2 < 0 || color1 > 255 || color2 > 255)
		return SendErrorMessage(playerid, "สีห้ามเกินตามนี้"); 
	...
	format(str, sizeof(str), "%s ได้เสกรถชื่อ %s ออกมา", ReturnName(playerid), ReturnVehicleName(vehicleid));
...
CMD:despawncar(playerid, params[])
{
	...
	if(VehicleInfo[vehicleid][eVehicleAdminSpawn] == false)
		return SendErrorMessage(playerid, "คุณไม่ได้สามารถลบรถที่ไม่ใช่รถเสกได้"); 
	
	format(str, sizeof(str), "%s ลบยานพาหนะ %s (%d)", ReturnName(playerid), ReturnVehicleName(vehicleid), vehicleid);
	SendAdminMessage(3, str);
...
CMD:pcar(playerid, params[])
{
	...
	{
		SendUsageMessage(playerid, "/pcar [ชื่อบางส่วน/ไอดี] [model id] [color1] [color2]");
		SendServerMessage(playerid, "เป็นการแอดยานพาหนะเข้าสู่ตัวละครและจพถูกเซฟไว้ในตัวละคร");
		return 1;
	}
	
	if (!IsPlayerConnected(playerb))
		return SendErrorMessage(playerid, "ผู้เล่นไม่ได้เชื่อมต่อกับเซฟเวอร์"); 
		
	if (e_pAccountData[playerb][mLoggedin] == false)
		return SendErrorMessage(playerid, "ผู้เล่นกำลังเข้าสู่ระบบ"); 
	
	if(modelid < 400 || modelid > 611)
		return SendErrorMessage(playerid, "ไม่มีไอดีที่ต่องการ");
		
	if(color1 < 0 || color2 < 0 || color1 > 255 || color2 > 255)
		return SendErrorMessage(playerid, "อย่าใช้สีมั่วไอ้สัส"); 
	...
	{
		SendErrorMessage(playerid, "%s ไม่สามารถแอดยานพาหนะเข้าไปในตัวละครได้เนื่องจากตัวละครดังกล่าวรถเต็มประดาแล้ว", ReturnName(playerb));
	}
	else
	{
		new insertQuery[256];
...
CMD:setcarparams(playerid, params[])
{
	...
	if(sscanf(params, "is[60]S()[60]", vehicleid, a_str, b_str))
	{
		SendUsageMessage(playerid, "/setcarparams [ไอดี รถ] [params]");
	...
	if(!strcmp(a_str, "engine"))
	{
		...
		{
			ToggleVehicleEngine(vehicleid, true); VehicleInfo[vehicleid][eVehicleEngineStatus] = true;
			format(str, sizeof(str), "%s ได้สตาร์ทเครื่องยนต์ %i", ReturnName(playerid), vehicleid);
		}
		else
		{
			ToggleVehicleEngine(vehicleid, false); VehicleInfo[vehicleid][eVehicleEngineStatus] = false;
			format(str, sizeof(str), "%s ได้ดับเครื่องยนต์ %i", ReturnName(playerid), vehicleid);
		}
	...
	else if(!strcmp(a_str, "lights"))
	{
		...
		format(str, sizeof(str), "%s ได้เปิดไฟรถ %i  %s.", ReturnName(playerid), vehicleid, (VehicleInfo[vehicleid][eVehicleLights] != true) ? ("off") : ("on"));
		SendAdminMessage(3, str);
	}
	else if(!strcmp(a_str, "lock"))
	{
		...
		format(str, sizeof(str), "%s %s รถไอดี %i.", ReturnName(playerid), (VehicleInfo[vehicleid][eVehicleLocked] != false) ? ("locked") : ("unlocked")); 
		SendAdminMessage(3, str);
	}
	else if(!strcmp(a_str, "health"))
	{
		...
		format(str, sizeof(str), "%s ตั้งค่ารถไอดี %i ให้เลือดรถเป็น  %.2f.", ReturnName(playerid), vehicleid, health);
		SendAdminMessage(3, str);
	}
...
CMD:editfaction(playerid, params[])
{
	...
	{
		SendUsageMessage(playerid, "/editfaction [เฟคชั่นไอดี] [configuration]");
	...
	if(!strcmp(oneString, "type"))
	{
		...
		if(sscanf(secString, "d", type))
		{
			...
			SendServerMessage(playerid, "1. Illegal, 2. Goverment"); 
			return 1;
		}
		
		if(type > 2 || type < 1)
			return SendErrorMessage(playerid, "ไม่มีประเภทที่คุณต้องการ"); 
		...
		if(type == 2) typeName = "Goverment";
		
		SendServerMessage(playerid, "คุณต้องค่าเฟคชั่นไอดี %d เป็นประเภท %s.", factionid, typeName);
	...
	else if(!strcmp(oneString, "alterrank"))
	{
		...
		if(sscanf(secString, "d", rankid))
		{
			...
			SendServerMessage(playerid, "การตั้งค่าในส่วนตรงนี้ยศที่ตั้งค่าไปนั้นจะสามารถแแก้ไขข้อมูลหรือเชิญคนอื่นเข้าร่วมเฟคชั่นได้"); 
			return 1;
		}
		
		if(rankid < 1 || rankid > 20)
			return SendErrorMessage(playerid, "ยศ/ต่ำแหน่ง มี 1-20");  
			
		FactionInfo[factionid][eFactionAlterRank] = rankid;
		SendServerMessage(playerid, "คุณตั้งค่าเฟคชั่นไอดี %d 'alter rank' เป็น %d", factionid, rankid);
	...
	else if(!strcmp(oneString, "joinrank"))
	{
		...
		if(sscanf(secString, "d", rankid))
		{
			...
			SendServerMessage(playerid, "เมื่อมีการเข้าร่วมเฟคชั่นจะยศ/ต่ำแหน่งเบื่องต้น จะให้ไปอยู่ที่ตั้งค่าไว้"); 
			return 1;
		}
		...
		SendServerMessage(playerid, "คุณตั้งค่าเฟคชั่นไอดี %d 'join rank' เป็น %d.", factionid, rankid);
	...
	else if(!strcmp(oneString, "chatrank"))
	{
		...
		if(sscanf(secString, "d", rankid))
		{
			...
			SendServerMessage(playerid, "จะให้ยศ/ตำแหน่งไหนขั้นไปที่สามารถให้พูดได้");
			return 1;
		}
		...
		SendServerMessage(playerid, "คุณตั้งค่าเฟคชั่นไอดี %d 'chat rank' เป็น %d", factionid, rankid);
	...
	else if(!strcmp(oneString, "towrank"))
	{
		...
		if(sscanf(secString, "d", rankid))
		{
			...
			SendServerMessage(playerid, "จะให้ยศ/ตำแหน่งไหนขึ้นไปสามารถพิมพ์ /towcars ได้");
			return 1;
		}
		...
		SendServerMessage(playerid, "คุณตั้งค่าเฟคชั่นไอดี %d 'tow rank' เป็น %d.", factionid, rankid); 
	...
	else if(!strcmp(oneString, "chatcolor"))
	{
		...
		if(sscanf(secString, "x", hexcolor))
		{
			...
			SendServerMessage(playerid, "ตัวอย่างการต้องค่า: 0x8D8DFFFF"); 
			return 1;
		}
		
		FactionInfo[factionid][eFactionChatColor] = hexcolor;
		SendServerMessage(playerid, "คุณตั้งค่าเฟคชั่นไอดี %d 'chat color' เป็น \"%x\". ", factionid, hexcolor);
	...
	else if(!strcmp(oneString, "spawn"))
	{
		...
		SendServerMessage(playerid, "เปลี่ยนจุดเกิดเฟคชั่นไอดี %d "); 
	}
	else return SendErrorMessage(playerid, "ผิดพลาด");
	return 1;
}
...
CMD:setpfaction(playerid, params[])
{
	...
	if(PlayerInfo[playerb][pFaction] != 0)
	{
		...
		format(detailStr, sizeof(detailStr), "{FFFFFF}%s ได้มีเฟคชั่นอยู่แล้วคุณแน่ใจใช่หรือไม่ที่จะดำเนินการต่อ", ReturnName(playerb));
	...
	format(str, sizeof(str), "%s ตั้งค่าให้ %s เป็นหัวหน้ากลุ่มเฟคชั่นไอดี %d.", ReturnName(playerid), ReturnName(playerb), factionid);
	SendAdminMessage(4, str);
	
	SendServerMessage(playerb, "คุณตั้งค่าให้เป็นหัวหน้ากลุ่มเฟคชั่นไอดี %d จากผู้ดูแลระบบ %s", factionid, ReturnName(playerid));
...
CMD:editproperty(playerid, params[])
{
	...
	{
		SendUsageMessage(playerid, "/editproperty [บ้านไอดี] [params]");
	...
	if(!PropertyInfo[id][ePropertyDBID] || id > MAX_PROPERTY)
		return SendErrorMessage(playerid, "ไม่มีไอดีบ้านที่ต้องการ");
	
	if(!strcmp(indx, "entrance"))
	{
		ConfirmDialog(playerid, "Confirmation", "คุณมั่นใจแล้วใช่ไหมที่จะเปลี่ยนจุดหน้าทางเข้า?", "OnEntranceChange", id); 
	}
	else if(!strcmp(indx, "interior"))
	{
		ConfirmDialog(playerid, "Confirmation", "คุณมั่นใจแล้วใช่ไหมที่จะเปลี่ยนจุดหน้าทางออก?", "OnInteriorChange", id); 
	}
	...
	{		
		...
		if(!FactionInfo[factionid][eFactionDBID] || factionid > MAX_FACTIONS)
			return SendErrorMessage(playerid, "ไม่มีไอดีเฟคชั่นที่ต้องการ");
			
		PropertyInfo[id][ePropertyFaction] = factionid;
		SaveProperty(factionid);
		
		SendServerMessage(playerid, "คุณตั้งค่าให้บ้านหลังไอดีที่ %i เป็นบ้านเฟคชั่นไอดี %i", id, factionid);
	}
	else if(!strcmp(indx, "type"))
	{
		...
		if(vartype > 3 || vartype < 1) 
			return SendErrorMessage(playerid, "สามารถตั้งค่าได้ 1-3");
		...
		SendServerMessage(playerid, "คุณตั้งค่าประเภทบ้านไอดี %i' เป็น %s", id, typeName);	
	}
	else if(!strcmp(indx, "marketprice"))
	{
		...
		if(price < 1)
			return SendErrorMessage(playerid, "การตั้งราคาขายของบ้านนั้นต้องไม่ต่ำกว่า $1"); 
			 
		PropertyInfo[id][ePropertyMarketPrice] = price;
		SaveProperty(id);
		
		SendServerMessage(playerid, "คุณตั้งค่าราคาขายบ้านไอดี %i เป็นราคา %d", id, price);
	}
	else if(!strcmp(indx, "level"))
	{
		...
		if(level < 1)
			return SendErrorMessage(playerid, "การตั้งค่าเลเวลในการซื้อบ้านต้องไม่ต่ำกว่า 1");
			
		PropertyInfo[id][ePropertyLevel] = level;
		SaveProperty(id);
		
		SendServerMessage(playerid, "คุณตั้งค่าเลเวลในการซื้อบ้านไอดี %i เป็นเลเวล %d", id, level);
	}
	else return SendServerMessage(playerid, "ผิดพลาด");	
	return 1;
}
...
CMD:makexmrcat(playerid, params[])
{
	...
	if(isnull(params))
	{
		...
		SendServerMessage(playerid, "นี่คือหมวดหมู่สถานีที่ระบุไว้ในรายการสถานี"); 
		return 1;
	}
	
	if(strlen(params) > 90 || strlen(params) < 3)
		return SendErrorMessage(playerid, "หมวดหมู่ของคุณยาวหรือสั้นเกินไป");
	...
	if(idx == 0)
		return SendErrorMessage(playerid, "เซิร์ฟเวอร์มีคุณสมบัติตามหมวดหมู่ความจุของ XMR แล้ว"); 
...
CMD:makexmrstation(playerid, params[])
{
	...
	if(!XMRCategoryInfo[category][eXMRID] || category > MAX_XMR_CATEGORY)
		return SendErrorMessage(playerid, "หมวดหมู่ที่คุณระบุไม่มีอยู่"); 
	...
	if(idx == 0)
		return SendErrorMessage(playerid, "เซิร์ฟเวอร์ตอบสนองความจุของสถานี XMR ได้แล้ว"); 
...
CMD:makebusiness(playerid, params[])
{
	...
	if(type_id > 7 || type_id < 1)
		return SendErrorMessage(playerid, "โปรดใส่ประเภทให้ถูกต้อง");
	...
	if(idx == 0)
		return SendErrorMessage(playerid, "คุณไม่สามารถสร้างกิจการได้มากกว่านี้แล้ว"); 
...
CMD:editbusiness(playerid, params[])
{
	...
	{
		SendUsageMessage(playerid, "/editbusiness [กิจการไอดี] [configuration]");
	...
	if(!BusinessInfo[id][eBusinessDBID] || id > MAX_BUSINESS)
		return SendErrorMessage(playerid, "ไม่มีไอดีกิจการ");
		
	if(!strcmp(astr, "1"))
	{
		ConfirmDialog(playerid, "Confirmation", "คุณมั่นใจใช่หรือไม่ที่จะเปลี่ยนจุดหน้าทางออกของกิจการ?", "OnBusinessInteriorChange", id); 
	}
	else if(!strcmp(astr, "2"))
	{
		ConfirmDialog(playerid, "Confirmation", "คุณมั่นใจใช่หรือไม่ที่จะเปลี่ยนจุดหน้าทางเข้าของกิจการ?", "OnBusinessEntranceChange", id); 
	}
	else if(!strcmp(astr, "type"))
	{
		...
		if(type == BusinessInfo[id][eBusinessType])
			return SendErrorMessage(playerid, "กิจการของคุณเป็นแบบนี้อยู่แล้ว"); 
		...
		if(BusinessInfo[id][eBusinessType] == BUSINESS_TYPE_BANK)
		{
			...
			SendServerMessage(playerid, "เนื่องจากคุณเปลี่ยนธุรกิจแล้วจากธนาคารเป็นแบบอื่น ทำให้ต้องลบ 'Bank Pickup'"); 
		}
		
		SendServerMessage(playerid, "คุณเปลี่ยนประเภทกิจการไอดี %i เป็นประเภท %i.", id, type);
		SaveBusiness(id);	
	}
	else if(!strcmp(astr, "level"))
	{
		...
		if(level < 1)
			return SendErrorMessage(playerid, "คุณไม่สามารถตั้งเลเวลต่ำกว่า 1 ได้"); 
			
		BusinessInfo[id][eBusinessLevel] = level;
		
		SendServerMessage(playerid, "คุณตั้งค่ากิจการไอดี %i ให้เลเวลการซื้อกิจการนั้นเป็น %i.", id, level);
		SaveBusiness(id);
	}
	else if(!strcmp(astr, "entrancefee"))
	{
		...
		if(sscanf(bstr, "i", fee))
			return SendUsageMessage(playerid, "/editbusiness %i entrancefee [จำนวน]", id);
			
		if(fee > 1500)
			return SendErrorMessage(playerid, "การตั้งค่าผ่านทางเข้ากิจการควรตั้งไม่เกิน $1,500");
			
		BusinessInfo[id][eBusinessEntranceFee] = fee;
		
		SendServerMessage(playerid, "คุณตั้งค่ากิจการไอดี %i ให้ค่าผ่านทางเข้าเป็น %i", id, fee);
		SaveBusiness(id);
	}
	else if(!strcmp(astr, "name"))
	{
		...
		if(sscanf(bstr, "s[90]", bizname))
		{
			...
			SendServerMessage(playerid, "คุณสามาใช้ Color text แบบนี้ได้ (i.e ~r~Red ~b~Blue) ในการตั้งชื่อกิจการ"); 
			return 1;
		}
		
		if(strlen(bizname) > 90)
			return SendErrorMessage(playerid, "ห้ามเกิน 90 ตัวอักษร");
			
		SendServerMessage(playerid, "คุณตั้งค่ากิจการไอดี %i ให้ชื่อจาก \"%s\" เป็น \"%s\". ", id, BusinessInfo[id][eBusinessName], bizname); 
	...
	{
		if(BusinessInfo[id][eBusinessType] != BUSINESS_TYPE_BANK)
			return SendErrorMessage(playerid, "กิจการนี้ไม่ใช่ประเภทธนาคาร"); 
			
		ConfirmDialog(playerid, "Confirmation", "คุณมั่นใจแล้วใช่ไหมที่จะตั้งค่า / เปลี่ยนจุด 'Bank Pickup' ?", "OnBusinessBankpickupChange", id);
	}
	else if(!strcmp(astr, "marketprice"))
	{
		...
		SendServerMessage(playerid, "คุณตั้งค่ากิจการไอดี %i ตั้งค่าราคาในการขายเป็น $%s.", id, MoneyFormat(price));
		SaveBusiness(id);
	}
	else return SendErrorMessage(playerid, "ผิดพลาด");
	return 1;
...
CMD:callpaycheck(playerid, params[])
{
	...
	ConfirmDialog(playerid, "Confirmation", "คุณมั่นใจใช่ไหมที่จะทำการชดเชยค่ารายชัวโมงในเวลานี้?\n\nการกระทำของคุณอาจจะผิดกฏของผู้ดูแลระบบได้", "OnCallPaycheck"); 
	return 1;
}
...
function:ClearReports(playerid, response, reports)
{
	if(response)
	{
		...
		format(str, sizeof(str), "%s ได้ลบการรายงาน %d", ReturnName(playerid), reports);
		SendAdminMessage(1, str); 
	}
...
function:OnSetFaction(playerid, response, playerb, factionid)
{
	if(response)
	{
		...
		format(str, sizeof(str), "%s ตั้งให้ %s เป็นหัวหน้ากลุ่มเฟคชั่นไอดี %d", ReturnName(playerid), ReturnName(playerb), factionid);
		SendAdminMessage(4, str);
		
		SendServerMessage(playerb, "คุณถูกตั้งให้เป็นหัวหน้ากลุ่มเฟคชั่นไอดี %d โดยผู้ดูแลระบบ %s", factionid, ReturnName(playerid));
...
function:OnGascanRefill(playerid, vehicleid, x, y, z)
{
	...
	{
		SendClientMessage(playerid, COLOR_ACTION, "มีการเคลื่อนย้ายยานพาหนะทำให้ต้องยกเลิกการเติมน้ำมันในทันที"); 
	...
	{
		SendClientMessage(playerid, COLOR_ACTION, "คุณได้ขยับตัวเองทำให้ต้องยกเลิกการเติมน้ำมันในทันที"); 
	...
	switch(VehicleInfo[vehicleid][eVehicleRefillCount])
	{
		...
		case 8:
		{
			...
			SendClientMessageEx(playerid, COLOR_ACTION, "คุณได้เติมน้ำมันสำรองให้รถรุ่น %s เพื่มขึ้นเป็น 50 ลิตร", ReturnVehicleName(vehicleid)); 
			SendNearbyMessage(playerid, 20.0, COLOR_EMOTE, "* %s ได้เติมน้ำมันสำรองให้รถรุ่น %s (สำเร็จ)", ReturnName(playerid, 0), ReturnVehicleName(vehicleid)); 
			return 1;
		}
	}
...
function:OnPropertyBoombox(playerid, response, propertyid, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
	if(response)
	{
		...
		SendServerMessage(playerid, "คุณได้ทำการซื้อ 'Boombox' เรียบร้อยแล้ว"); 
	...
	else
	{
		...
		SendServerMessage(playerid, "ยกเลิกการกระทำ"); 
	}
	return 1;
}
...
function:OnRemoveBoombox(playerid, response, propertyid)
{
	if(response)
	{
		...
		SendServerMessage(playerid, "คุณได้ลบ 'Boombox' ออกจากบ้านของคุณแล้ว");
		SaveProperty(propertyid);
	}
	else return SendServerMessage(playerid, "ยกเลิกการกระทำ");
	return 1;
}
...
function:OnBusinessInteriorChange(playerid, response, businessid)
{
	if(response)
	{
		...
		SendServerMessage(playerid, "คุณได้เปลี่ยนจุดหน้าทางเข้าของกิจการไอดี", businessid);
		...
		foreach(new i : Player) if(IsPlayerInBusiness(i) == businessid)
		{
			...
			SendServerMessage(i, "ธุรกิจของคุณได้รับการแก้ไข"); 
		}
		...
		if(BusinessInfo[businessid][eBusinessType] == BUSINESS_TYPE_BANK)
		{
			if(IsValidDynamicPickup(BusinessInfo[businessid][eBusinessBankPickup]))
			{
				...
				SendServerMessage(playerid, "'Bank Pickup' ถูกลบเนื่องจากมีการเปลี่ยนภายในจึงจำเป็นต้องสร้างใหม่"); 
			}
...
function:OnBusinessEntranceChange(playerid, response, businessid)
{
	if(response)
	{
		...
		SendServerMessage(playerid, "คุณได้เปลี่ยนหน้าทางเข้ากิจการไอดี %i", businessid);
		SaveBusiness(businessid); 
	}
	else return SendServerMessage(playerid, "ยกเลิกการกระทำ");
	return 1;
...
function:OnBusinessBankpickupChange(playerid, response, businessid)
{
	if(response)
	{
		...
		SendServerMessage(playerid, "ได้เปลี่ยนจุด 'Bank Pickup' กิจการไอดี %d", businessid); 
		SaveBusiness(businessid);
	}
	else return SendServerMessage(playerid, "ยกเลิกการกระทำ");
	return 1;
...
function:OnSellBusiness(playerid, response, businessid)
{
	if(response)
	{
		...
		PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
		SendClientMessageEx(playerid, COLOR_ACTION, "คุณได้ทำการขายกิจการของคุณเรียบร้อยแล้ว $%s.", MoneyFormat(totalPay)); 
...
function:OnPlayerExitDealership(playerid, response)
{
	if(response)
	{
		SendServerMessage(playerid, "คุณออกจากร้านขายรถ");
...
function:OnPlayerVehiclePurchase(playerid, id, plates[], Float:x, Float:y, Float:z, Float:a)
{
	...
	SendClientMessageEx(playerid, 0xB9E35EFF, "PROCESSED: คุณได้ทำการซื้อรถรุ่น %s ด้วยเงิน $%s  เรียบร้อยแล้ว", ReturnVehicleName(vehicleid), MoneyFormat(DealershipTotalCost[playerid]));
...
function:OnPlayerPurchaseWeapon(playerid, response, weapon, ammo, price)
{
	if(response)
	{
		if(price > PlayerInfo[playerid][pMoney])
			return SendErrorMessage(playerid, "คุณไม่สามารถซื้ออาวุธได้เนื่องจากคุณมีเงินไม่เพียงพอ (ราคา: $%s, เงินที่มี: $%s)", MoneyFormat(price), MoneyFormat(PlayerInfo[playerid][pMoney]));
		...
		format(str, sizeof(str), "%s ซื้ออาวุธ %s และกระสุน %d ชุด", ReturnName(playerid), ReturnWeaponName(weapon), ammo);
		SendAdminMessage(1, str); 
	}
	return 1;
}
...
function:OnPlayerPurchaseArmor(playerid, response)
{
	if(response)
	{
		if(2000 > PlayerInfo[playerid][pMoney])
			return SendErrorMessage(playerid, "คุณไม่สามารถซื้อเสื้อเกราะได้เนื่องจากคุณมีเงินไม่เพียงพอ (ราคา: $2,000, เงินที่มี: $%s)", MoneyFormat(PlayerInfo[playerid][pMoney]));
		...
		format(str, sizeof(str), "%s ซื้อเสื้อเกราะ", ReturnName(playerid));
		SendAdminMessage(1, str); 
	}
	return 1;
}
...
{
	SendPoliceMessage(COLOR_COP, "[WANTED] ผู้ต้องหา: %s ผู้เขียนรายงาน: %s สาเหตุ: %s", ReturnName(playerb), ReturnName(playerid), charge);
...
function:OnPlayerPurchaseAmmo(playerid, response, weaponid, ammo, price)
{
	if(response)
	{
		...
		format(str, sizeof(str), "%s ซื้อกระสุนจำนวน %d ชุด ด้วยเงิน %s", ReturnName(playerid), ammo, ReturnWeaponName(weaponid));
		SendAdminMessage(1, str); 
	}
	return 1;
}
...
function:Query_InsertFaction(playerid, varName, varAbbrev, idx)
{
	...
	SendServerMessage(COLOR_RED, "สร้างเฟคชั่นสำเร็จโปรดพิมพ์ \"/editfaction\".เพื่อทำการแก้ไข "); 
	return 1;
}
...
function:Query_InsertBusiness(playerid, newid, type)
{
	...
	SendServerMessage(playerid, "คุณได้สร้างกิจการไอดี %i พิมพ์ \"/editbusiness\". เพื่อแก้ไข ", newid); 
	return 1;
}
...
function:Query_LoadFactions()
{
	...
	for (new i = 0; i < rows && i < MAX_FACTIONS; i ++)
	{
		...
		FactionInfo[i+1][eFactionJob] = cache_get_field_content_int(i, "FactionJob", ourConnection);
...
function:Query_LoadPrivateVehicle(playerid)
{
	...
	SendClientMessageEx(playerid, COLOR_DARKGREEN, "%s ถูกนำออกมาและอยู่ที่จอดรถในจุดที่เราไปเก็บไว้แล้ว", ReturnVehicleName(vehicleid));
	...
	SendClientMessage(playerid, 0xFF00FFFF, "Hint: ไปตามจุดที่เราได้ทำการมาร์ากไว้ให้");
...
function:Query_AddPlayerVehicle(playerid, playerb)
{
	...
	SendServerMessage(playerb, "คุณได้รับยานพาหนะเข้ามาในตัวของคุณจาก ผู้ดูแลระบบ %s", ReturnName(playerid));
	SendServerMessage(playerid, "คุณได้มอบยานพาหนะให้กับ %s", ReturnName(playerb));
...
function:OnPropertyCreate(playerid, type)
{
	...
	format(str, sizeof(str), "%s ได้สร้างบ้าน ไอดี:%i ขึ้นมา", ReturnName(playerid), cache_insert_id());
	SendAdminMessage(4, str);
	
	SendClientMessage(playerid, COLOR_RED, "[ ! ]{FFFFFF} พิมพ์ \"/editproperty\" เพื่อแก้ไขบ้าน");
	return 1;
}
...
function:OnEntranceChange(playerid, response, property)
{
	if(response)
	{
		...
		SendServerMessage(playerid, "คุณได้เปลี่ยนจุดหน้าทางเข้าบ้านไอดี %d", property);
	}
	return 1;
}
...
function:OnInteriorChange(playerid, response, property)
{
	if(response)
	{
		...
		foreach(new i : Player) if(IsPlayerInProperty(i) == property)
		{
			...
			SendServerMessage(i, "แก้ไขคุณสมบัติที่คุณอยู่"); 
		}
		
		SaveProperty(property);
		SendServerMessage(playerid, "ได้เปลี่ยนจุดภายในบ้านไอดี %d", property);
	}
	return 1;
}
...
{
	SendServerMessage(playerid, "%s ได้ถูกผู้ดูแลระบบส่งคุกแอดมินเรียบร้อย", jailing); 
...
{
	SendServerMessage(playerid, "%s ได้ถูกแบบออกจากเซืฟเวอร์เรียบร้อย", banning);
...
function:OnXMRCategory(playerid, newid, cat[])
{
	...
	SendServerMessage(playerid, "ได้สร้างสถาณี XMR category. \"%s\" (ID: %i)", cat, newid); 
	return 1;
}
...
function:OnXMRStation(playerid, category, newid, url[], name[])
{
	...
	SendServerMessage(playerid, "ได้สร้างสถาณี XMR station. \"%s\" (ID: %i)", name, newid);
...
function:OnPhoneSMS(playerid, type, playerb, text[])
{
	switch(type)
	{
		case 1:
		{
			SendClientMessage(playerid, COLOR_GREY, "[ ! ] การส่งข้อความล้มเหลว"); 
		...
		case 3:
		{
			...
			SendClientMessage(playerid, COLOR_GREY, "[ ! ] ส่งข้อความของคุณแล้ว"); 
			
			if(strlen(text) > 80)
			{
				SendClientMessageEx(playerb, COLOR_YELLOWEX, "SMS: %.80s ...", text, PlayerInfo[playerid][pPhone]); 
				SendClientMessageEx(playerb, COLOR_YELLOWEX, "SMS: ...%s, ส่งโดยเบอร์: %i", text[80], PlayerInfo[playerid][pPhone]); 
				printf("Call ;");
			}
			else SendClientMessageEx(playerb, COLOR_YELLOWEX, "SMS: %s, ส่งโดยเบอร์: %i", text, PlayerInfo[playerid][pPhone]);
...
{
	if(!cache_num_rows())
		return SendClientMessage(playerid, COLOR_RED, "[ ! ] {FFFFFF}ผู้เล่นคนี้ไม่ได้ถูกส่งคุกแอดมินเลย");
	...
	for(new i = 0; i < rows; i++)
	{
		...
		SendClientMessageEx(playerid, COLOR_ACTION, "[%s] %s ได้ถูกส่งคุกแอดมินโดย %s เวลา %d นาที สาเหตุ: %s", Date, JailedName, JailedBy, Time, Reason);
	}
...
{
	return SendClientMessage(playerid, COLOR_RED, "ACCESS DENIED:{FFFFFF} คุณไม่ได้รับอนุญาติให้ใช้คำสั่งนี้");
}
...
stock ShowCharacterStats(playerid, playerb)
{
	...
	SendClientMessageEx(playerb, COLOR_GRAD2, "การเงิน: เงินในตัว:[$%s] เงินในธนาคาร:[$%s] เงินรายชัวโมง:[$%s]", MoneyFormat(PlayerInfo[playerid][pMoney]), MoneyFormat(PlayerInfo[playerid][pBank]), MoneyFormat(PlayerInfo[playerid][pPaycheck]));
	...
	if(PlayerInfo[playerb][pAdmin])
	{
		...
		SendClientMessageEx(playerb, COLOR_GRAD2, "การเชื่อมต่อ: IP:[%s] ออนไลน์ล่าสุด:[%s] ชัวโมงออนไลน์:[%d Minutes]", ReturnIP(playerid), ReturnLastOnline(playerid), PlayerInfo[playerid][pLastOnlineTime]);
...
stock NotifyVehicleOwner(vehicleid)
{
	...
	if(playerid != INVALID_PLAYER_ID)
	{
		SendClientMessage(playerid, COLOR_YELLOWEX, "SMS: ยานพาหนะของท่านได้รับการงัดรถอยู่ในขณะนี้");
	}
	else return 1;
	return 1;
}
...
stock PlayXMRStation(playerid, vehicleid = INVALID_VEHICLE_ID, propertyid = 0, bool:disableXMR = false)
{
		...
		if(vehicleid != INVALID_VEHICLE_ID)
		{
			foreach(new i : Player)
			{
				if(IsPlayerInVehicle(i, vehicleid))
				{
					...
					SendClientMessage(i, COLOR_RED, "ปิดวิทยุ"); 
				}
			}
	...
	if(vehicleid != INVALID_VEHICLE_ID)
	{
		format(string, sizeof(string), "> %s เปิดวิทยุ %s.", ReturnName(playerid, 0), XMRStationInfo[SubXMRHolder[playerid]][eXMRStationName]);
		...
		foreach(new i : Player)
		{
			if(IsPlayerInVehicle(i, vehicleid))
			{
				...
				SendClientMessageEx(i, COLOR_RED, "เปลี่ยนคืนวิทยุ %s.", XMRStationInfo[SubXMRHolder[playerid]][eXMRStationName]); 
			}
		}
	...
	if(propertyid != 0)
	{
		format(string, sizeof(string), "> %s ได้เปิดวิทยุเป็นสถาณี %s.", ReturnName(playerid, 0), XMRStationInfo[SubXMRHolder[playerid]][eXMRStationName]);
		SetPlayerChatBubble(playerid, string, COLOR_ACTION, 20.0, 3000);
		SendClientMessage(playerid, 0x88AA62FF, string);
		...
		foreach(new i : Player)
		{
			...
			{
				PlayAudioStreamForPlayer(i, XMRStationInfo[SubXMRHolder[playerid]][eXMRStationURL]); 
				SendClientMessageEx(i, COLOR_RED, "เปลี่ยนคลื่นวิทยุ %s.", XMRStationInfo[SubXMRHolder[playerid]][eXMRStationName]); 
			}
		}
...
stock SendBusinessType(playerid, id)
{
	switch(BusinessInfo[id][eBusinessType])
	{
		...
		{
			SendClientMessageEx(playerid, COLOR_DARKGREEN, "ยินดีตอยรับเข้าสู่ร้าน  %s", BusinessInfo[id][eBusinessName]);
...
stock ShowPlayerDamages(damageid, playerid, adminView)
{
	...
	if (TotalPlayerDamages[damageid] < 1)
		return ShowPlayerDialog(playerid, DIALOG_DEFAULT, DIALOG_STYLE_LIST, caption, "ไม่ได้รับความเสียหาย...", "<<", ""); 
